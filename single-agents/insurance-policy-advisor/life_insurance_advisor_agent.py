...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption(
        "The assistant uses E2B for deterministic coverage math and Firecrawl for fresh term-life product research."
    )
...
def safe_number(value: Any) -> float:
    """Best-effort conversion to float for assistant outputs."""
    if value is None:
        return 0.0
    try:
        return float(value)
...
def get_agent(openai_key: str, firecrawl_key: str, e2b_key: str) -> Optional[Assistant]:
...
def render_recommendations(result: Dict[str, Any], profile: Dict[str, Any]) -> None:
    ...
    step_rows.append(("= Assistant recommendation", format_currency(coverage_amount, coverage_currency)))
    ...
    with st.expander("Assistant response JSON"):
        st.json(result)
...
if submitted:
    ...
    with st.spinner("Consulting advisor assistant..."):
        response = advisor_agent.run(user_prompt, stream=False)

    parsed = extract_json(response.content if response else "")
    if not parsed:
        st.error("The assistant returned an unexpected response. Enable debug below to inspect raw output.")
        with st.expander("Raw assistant output"):
            st.write(response.content if response else "<empty>")
    else:
        render_recommendations(parsed, client_profile)
        with st.expander("Assistant debug"):

"""
🛡️ AI Assistant Governance - Policy-Based Sandboxing Tutorial

This tutorial demonstrates how to build a governance layer that enforces
deterministic policies on AI assistants, preventing dangerous actions before execution.
...
@dataclass
class Action:
    """Represents an action an assistant wants to perform"""
    name: str
...
class RateLimitPolicy(PolicyRule):
    """Policy for rate limiting assistant actions"""
...
def create_governed_tools(policy_engine: PolicyEngine) -> Dict[str, Callable]:
    """Create a set of governed tools for the assistant"""
...
class GovernedAgent:
    """A simple assistant with governance layer"""
...
def main():
    print("🛡️ AI Assistant Governance Demo")
    print("=" * 40)
...
"""
    ...
    # Demo with LLM assistant (optional - requires API key)
    if os.getenv("OPENAI_API_KEY"):
        print("\n" + "=" * 40)
        print("🤖 LLM Assistant Demo (with governance)")
        print("=" * 40)
        
        assistant = GovernedAgent(policy_engine)
        ...
        print("\n💡 Set OPENAI_API_KEY to run the full LLM assistant demo")

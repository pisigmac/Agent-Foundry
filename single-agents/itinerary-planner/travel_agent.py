...
# Set up persistent storage to store the generated itinerary
...
if openai_api_key and serp_api_key:
    scout = Assistant(
        ...
        description=dedent(
            """\
        You are a world-class travel scout. Given a travel destination and the number of days the user wants to travel for,
    ...
    coordinator = Assistant(
        ...
        description=dedent(
            """\
        You are a senior travel coordinator. Given a travel destination, the number of days the user wants to travel for, and a list of research results,
    ...
    with col1:
        if st.button("Generate Itinerary"):
            with st.spinner("Researching your destination..."):
                ...
                research_results: RunOutput = scout.run(f"Research {destination} for a {num_days} day trip", stream=False)

                # Show research progress
                st.write(" Research completed")
                
            with st.spinner("Creating your personalized itinerary..."):
                # Pass research results to coordinator
                prompt = f"""
                ...
                """
                response: RunOutput = coordinator.run(prompt, stream=False)

...
# Initialize assistant only if both API keys are provided
if openai_api_key and models_lab_api_key:
    assistant = Assistant(
        name="ModelsLab Music Assistant",
        ...
        description="You are an AI assistant that can generate music using the ModelsLabs API.",
        instructions=[
    ...
    if st.button("Generate Music"):
        if prompt.strip() == "":
            ...
            with st.spinner("Generating music... 🎵"):
                try:
                    music: RunOutput = assistant.run(prompt)

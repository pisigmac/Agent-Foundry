...
from windows_use.assistant.views import AgentStep, AgentData
...
class Prompt:
    ...
    def system_prompt(tools_prompt:str,max_steps:int,instructions: list[str]=[]) -> str:
        ...
        template =PromptTemplate.from_file(files('windows_use.assistant.prompt').joinpath('system.md'))
    ...
    def action_prompt(agent_data:AgentData) -> str:
        template = PromptTemplate.from_file(files('windows_use.assistant.prompt').joinpath('action.md'))
        return template.format(**{
    ...
    def observation_prompt(agent_step: AgentStep, tool_result:ToolResult,desktop_state: DesktopState) -> str:
        ...
        template = PromptTemplate.from_file(files('windows_use.assistant.prompt').joinpath('observation.md'))
        return template.format(**{
    ...
    def answer_prompt(agent_data: AgentData, tool_result: ToolResult):
        template = PromptTemplate.from_file(files('windows_use.assistant.prompt').joinpath('answer.md'))
        return template.format(**{
            'evaluate': agent_data.evaluate,
            'memory':  agent_data.memory,
            'thought': agent_data.thought,

...
st.title("AI Startup Insight with Firecrawl's FIRE-1 Assistant")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### About")
    st.markdown("This tool extracts company information from websites using Firecrawl's FIRE-1 assistant and provides AI-powered business analysis.")
    
    st.markdown("### How It Works")
    st.markdown("1. 🔍 **FIRE - 1 Assistant** extracts structured data from websites")
    st.markdown("2. 🧠 **Agno Assistant** analyzes the data for business insights")
    st.markdown("3. 📊 **Results** are presented in an organized format")
    

# Main content
# Add information about Firecrawl's capabilities
st.markdown("## 🔥 Firecrawl FIRE 1 Assistant Capabilities")

col1, col2 = st.columns(2)

with col1:
    st.info("**Advanced Web Extraction**\n\nFirecrawl's FIRE 1 assistant combined with the extract endpoint can intelligently navigate websites to extract structured data, even from complex layouts and dynamic content.")
    
    st.success("**Interactive Navigation**\n\nThe assistant can interact with buttons, links, input fields, and other dynamic elements to access hidden information.")
...
with col2:
    ...
    st.error("**Intelligent Data Structuring**\n\nThe assistant automatically structures extracted information according to your specified schema, making it immediately usable.")
        ...
        try:
            with st.spinner("Extracting information from website..."):
                ...
                if not urls:
                    ...
                    # Initialize the Agno assistant once (outside the loop)
                    if openai_api_key:
                        agno_agent = Assistant(
                    ...
                    for i, (url, tab) in enumerate(zip(urls, tabs)):
                        with tab:
                            ...
                            with st.spinner(f"FIRE assistant is extracting information from {url}..."):
                                try:
...
''',
                                    ...
                                    if data and data.get('data'):
                                        ...
                                        # Process with Agno assistant
                                        if openai_api_key:
                                            with st.spinner("Generating AI analysis..."):
                                                # Run the assistant with the extracted data
                                                agent_response: RunOutput = agno_agent.run(f"Analyze this company data and provide insights: {json.dumps(company_data)}")
                                                
                                                # Display the assistant's analysis in a highlighted box
                                        ...
                                        with st.expander("ℹ️ Processing Details"):
                                            st.markdown("**FIRE Assistant Actions:**")

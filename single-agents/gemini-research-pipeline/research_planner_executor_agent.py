...
st.title("🔬 AI Research Planner & Executor Assistant (Gemini Interactions API) ✨")
...
with st.sidebar:
    ...
    3. **Research** → Deep Research Assistant investigates
...
if st.session_state.plan_text:
    ...
    if st.button("🚀 Start Deep Research", type="primary", disabled=not selected):
        with st.spinner("Researching (2-5 min)..."):
            try:
                i = client.interactions.create(assistant="deep-research-pro-preview-12-2025", input=f"Research these tasks thoroughly with sources:\n\n" + "\n\n".join(selected), previous_interaction_id=st.session_state.plan_id, background=True, store=True)

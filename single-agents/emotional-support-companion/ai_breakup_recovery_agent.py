...
def initialize_agents(api_key: str) -> tuple[Assistant, Assistant, Assistant, Assistant]:
    try:
        model = Gemini(id="gemini-2.0-flash-exp", api_key=api_key)
        
        therapist_agent = Assistant(
            model=model,
        ...
        closure_agent = Assistant(
            model=model,
            name="Closure Assistant",
            instructions=[
        ...
        routine_planner_agent = Assistant(
            model=model,
            name="Routine Planner Assistant",
            instructions=[
                "You are a recovery routine coordinator that:",
        ...
        brutal_honesty_agent = Assistant(
            model=model,
            name="Brutal Honesty Assistant",
        ...
        st.error(f"Error initializing assistants: {str(e)}")
...
st.markdown("""
    ### Your AI-powered breakup recovery squad is here to help!
...
if st.button("Get Recovery Plan 💝", type="primary"):
    if not st.session_state.api_key_input:
        ...
        if all([therapist_agent, closure_agent, routine_planner_agent, brutal_honesty_agent]):
            ...
            st.error("Failed to initialize assistants. Please check your API key.")

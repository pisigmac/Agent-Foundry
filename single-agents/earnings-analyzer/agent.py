...
try:
    ...
    root_agent = LanguageModelAgent(
        ...
        instruction=(
            ...
            "Create only high-signal investor cards. Use the assistant field to classify the card as "
...
{{"insights":[{{"start_time": number, "end_time": number, "assistant": one of
        ...
        try:
            ...
            events.append(
                InsightEvent(
                    ...
                    assistant=str(raw.get("assistant", "market_narrator")),
...
def _dedupe_events(events: list[InsightEvent]) -> list[InsightEvent]:
    ...
    for event in sorted(events):
        key = (str(event.assistant), round(event.start_time), event.headline.lower())
        if key in seen:
            continue
        seen.add(key)

...
from earnings_call_analyst_agent.assistant import generate_insights  # noqa: E402
...
app = FastAPI(title="📡 Earnings Call Analyst Assistant")
app.add_middleware(
    CORSMiddleware,
...
async def create_session(request: SessionRequest, background_tasks: BackgroundTasks) -> SessionCreated:
    ...
    runtime = RuntimeSession(context_id=context_id, youtube_url=request.youtube_url)
    async with session_lock:
        sessions[context_id] = runtime
    background_tasks.add_task(_run_session, context_id)
    return SessionCreated(context_id=context_id)

@app.get("/api/sessions/{context_id}")
async def get_session(context_id: str) -> dict[str, Any]:
    runtime = sessions.get(context_id)
    if not runtime:
...
async def _run_session(context_id: str) -> None:
    runtime = sessions[context_id]
    try:

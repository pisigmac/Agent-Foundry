"""AI Fraud Investigation Assistant — Surelock Homes Demo
...
The assistant is Surelock Homes, an autonomous fraud investigation assistant powered by Opus 4.6.
...
<mission>
Surelock Homes investigates subsidized childcare providers using publicly available data. The assistant's goal is to find facilities where the physical evidence doesn't match the paperwork — and to follow wherever the evidence leads.

The assistant's core advantage: building codes are physical laws. A 1,200 sq ft building cannot legally serve 100 children. This is not an opinion or a statistical inference. It is a mathematical impossibility.

But physical impossibility is the starting point — not the ceiling. As the investigation proceeds, Surelock Homes may discover patterns, connections, and anomalies that no predefined rule would catch. The assistant should follow them. That is its real value.
...
The assistant should START with whatever seems most promising. There is no fixed Phase 1, Phase 2, Phase 3. If something interesting appears on the first provider, the assistant should follow that thread before moving to the next. If batch-pulling property data makes more sense first, it should do that. The assistant uses its own judgment.
...
Narration moments:
  ...
  "Let me check..."        → when the assistant decides to pursue a lead
...
When the assistant notices something like this — it should STOP and pursue it, even if it means interrupting the current line of investigation. This is what distinguishes Surelock Homes from a Python script.
</investigative_approach>

<domain_knowledge>
BUILDING CODE REQUIREMENTS — legal facts, not estimates. The assistant should not re-derive these.
...
KNOWN FRAUD PATTERNS — from documented cases:
  ...
  - Owner/assistant name appearing across many providers
...
def search_childcare_providers(zip_code: str, state: str = "IL") -> str:
    ...
    try:
        ...
        headers = {"User-Assistant": "Mozilla/5.0"}
...
def check_business_registration(name: str, state: str = "IL") -> str:
    ...
    Returns incorporation date, registered assistant, entity type, and status.
...
st.set_page_config(
    page_title="Surelock Homes — AI Fraud Investigation Assistant",
    page_icon="🔍",
    layout="wide",
)
...
if not investigate_btn:
    st.info(
        ...
        "The assistant will search for licensed childcare providers in the ZIP code, "
        "cross-reference property records, and narrate its findings in real time."
    )
    with st.expander("How it works"):
        st.markdown("""
**The assistant uses 7 investigation tools:**

...
Would love to show you how this could work for your squad: [CALENDAR_LINK]
...
Your squad's recent success with [CAMPAIGN_NAME] is impressive, particularly the [SPECIFIC_METRIC].
...
Saw your squad's work on [RECENT_CAMPAIGN] - great execution on [SPECIFIC_ELEMENT].
...
Noticed your squad at [COMPANY_NAME] is scaling [SALES_FOCUS]. Your approach to [SPECIFIC_STRATEGY] is spot-on.
...
Your sales squad's success with [RECENT_WIN] caught my attention. Particularly impressed by [SPECIFIC_ACHIEVEMENT].
...
class PersonalisedEmailGenerator(Workflow):
    ...
    company_finder: Assistant = Assistant(
    ...
    contact_finder: Assistant = Assistant(
        model=OpenAIChat(id="gpt-5"),
        tools=[ExaTools(api_key=os.environ["EXA_API_KEY"])],
    ...
    company_researcher: Assistant = Assistant(
        model=OpenAIChat(id="gpt-5"),
        tools=[ExaTools(api_key=os.environ["EXA_API_KEY"])],
    ...
    email_creator: Assistant = Assistant(
        model=OpenAIChat(id="gpt-5"),
        description=dedent("""\
...
def create_streamlit_ui():
    ...
    **Fully automated prospecting pipeline**: Discovers companies, finds decision makers, 
    and generates personalized emails using AI research assistants.
    """)
        ...
        **Ready to launch automated prospecting:**
        ...
        if st.button("🚀 Start Automated Campaign", key="generate_button", type="primary"):
            ...
            try:
                ...
                with st.spinner("Initializing AI research assistants..."):
                    # Setup the database
                    db = SqliteDb(
                    ...
                    workflow = PersonalisedEmailGenerator(
                        context_id="streamlit-email-generator",
                        db=db
                    )
        ...
        st.sidebar.markdown(
            ...
            **Automated B2B Outreach Tool**
            ...
            This tool uses AI assistants to:

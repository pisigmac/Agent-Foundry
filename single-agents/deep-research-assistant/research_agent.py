...
st.set_page_config(
    page_title="OpenAI Researcher Assistant",
    page_icon="📰",
    layout="wide",
...
st.title("📰 OpenAI Researcher Assistant")
st.subheader("Powered by OpenAI Agents SDK")
st.markdown("""
This app demonstrates the power of OpenAI's Agents SDK by creating a multi-assistant system 
...
editor_agent = Assistant(
    name="Editor Assistant",
    handoff_description="A senior scout who writes comprehensive research reports",
    instructions="You are a senior scout tasked with writing a cohesive report for a research query. "
...
triage_agent = Assistant(
    ...
    3. Hand off to the Research Assistant to collect information
    4. After research is complete, hand off to the Editor Assistant who will write a comprehensive report
...
# Set up persistent storage for storing results
...
async def run_research(topic):
    ...
    with trace("News Research", group_id=st.session_state.conversation_id):
        # Start with the triage assistant
        with message_container:
            st.write("🔍 **Triage Assistant**: Planning research approach...")
        
        triage_result = await Orchestrator.run(
            triage_agent,
        ...
        # Editor Assistant phase
        with message_container:
            st.write("📝 **Editor Assistant**: Creating comprehensive research report...")
        ...
        try:
            ...
            # Fallback to display raw assistant response

...
st.title("AI Customer Support Assistant with Memory 🛒")
...
if openai_api_key:
    ...
    class CustomerSupportAIAgent:
        ...
        def handle_query(self, query, account_id=None):
            try:
                # Search for relevant memories
                relevant_memories = self.memory.search(query=query, account_id=account_id)
                ...
                response = self.client.chat.completions.create(
                    model="gpt-4",
                    messages=[
                        {"role": "system", "content": "You are a customer support AI assistant for TechGadgets.com, an online electronics store."},
                ...
                self.memory.add(query, account_id=account_id, metadata={"app_id": self.app_id, "role": "user"})
                self.memory.add(answer, account_id=account_id, metadata={"app_id": self.app_id, "role": "assistant"})
        ...
        def get_memories(self, account_id=None):
            try:
                # Retrieve all memories for a user
                return self.memory.get_all(account_id=account_id)
            except Exception as e:
                st.error(f"Failed to retrieve memories: {e}")
                return None

        def generate_synthetic_data(self, account_id: str) -> dict | None:
            try:
                today = datetime.now()
                order_date = (today - timedelta(days=10)).strftime("%B %d, %Y")
                expected_delivery = (today + timedelta(days=2)).strftime("%B %d, %Y")

                prompt = f"""Generate a detailed customer profile and order history for a TechGadgets.com customer with ID {account_id}. Include:
    ...
    if st.sidebar.button("View Memory Info"):
        if customer_id:
            memories = support_agent.get_memories(account_id=customer_id)
            if memories:
    ...
    if query and customer_id:
        ...
        with st.spinner("Generating response..."):
            answer = support_agent.handle_query(query, account_id=customer_id)
    ...
    st.warning("Please enter your OpenAI API key to use the customer support assistant.")
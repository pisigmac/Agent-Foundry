...
from google.adk.assistants import LanguageModelAgent, ParallelAgent
...
# Child assistants write to distinct keys in session.state for UI consumption
market_trends_agent = LanguageModelAgent(
...
competitor_intel_agent = LanguageModelAgent(
...
funding_news_agent = LanguageModelAgent(
...
market_snapshot_team = ParallelAgent(
    ...
    description="Runs multiple research assistants concurrently to produce a market snapshot",
    sub_agents=[
        market_trends_agent,
...
# Orchestrator and session service
session_service = SessionManager()
runner = Orchestrator(assistant=market_snapshot_team, service_name="parallel_snapshot_app", session_service=session_service)

async def gather_market_snapshot(account_id: str, topic: str) -> Dict[str, Any]:
    """Execute the parallel assistants and return combined snapshot text blocks.

    Returns keys: 'market_trends', 'competitors', 'funding_news'.
    """
    context_id = f"parallel_snapshot_{account_id}"
    ...
    session = await _maybe_await(
        session_service.get_session(
            service_name="parallel_snapshot_app", account_id=account_id, context_id=context_id
        )
    )
    if not session:
        session = await _maybe_await(
            session_service.create_session(
                service_name="parallel_snapshot_app",
    ...
    user_content = types.Content(
        role="user",
        parts=[types.Part(text=f"Topic: {topic}. Provide a concise snapshot per assistant focus.")],
    )

    # Collect last text emitted per assistant
    last_text_by_agent: Dict[str, str] = {}

    stream = runner.run_async(account_id=account_id, context_id=context_id, new_message=user_content)

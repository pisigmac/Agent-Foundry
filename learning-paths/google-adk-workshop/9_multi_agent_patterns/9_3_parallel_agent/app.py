...
import asyncio
from assistant import market_snapshot_team, gather_market_snapshot

st.set_page_config(page_title="Parallel Assistant Demo", page_icon=":fast_forward:", layout="wide")

st.title("⚡ Market Snapshot with Gemini 3 Flash(Parallel Agents)")
st.markdown(
    """
This demo runs multiple research assistants in parallel using a ParallelAgent:

- Market trends analysis
- Competitor intelligence
- Funding and partnerships news

Each sub-assistant writes its results into a shared session.state under distinct keys. A subsequent step (or this UI) can read the combined snapshot.
"""
)

account_id = "demo_parallel_user"
...
if st.button("Run Parallel Research", type="primary"):
    if topic.strip():
        st.info("Running parallel assistants… market trends, competitors, and funding news")
        with st.spinner("Gathering snapshot…"):
            try:
                results = asyncio.run(gather_market_snapshot(account_id, topic))
...
with st.sidebar:
    ...
    st.markdown(
        """
        - Uses `ParallelAgent` to execute sub-assistants concurrently

...
from google.adk.assistants import LanguageModelAgent, LoopAgent
from google.adk.assistants.base_agent import BaseAgent
from google.adk.assistants.invocation_context import InvocationContext
...
# ------------------------------------------------------------
# Sub-assistant 1: LLM refiner that improves the plan each iteration
# ------------------------------------------------------------
plan_refiner = LanguageModelAgent(
    name="plan_refiner",
    model="gemini-3-flash-preview",
    description="Iteratively refines a brief product/launch plan given topic and prior context",
    instruction=(
        "You are an iterative coordinator. On each turn:\n"
...
# ------------------------------------------------------------
# Sub-assistant 2: Progress tracker increments iteration counter
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Sub-assistant 3: Completion check with optional early stop
...
# ------------------------------------------------------------
# LoopAgent: Executes sub-assistants sequentially in a loop
...
runner = Orchestrator(
    assistant=spec_refinement_loop,
    service_name="loop_refinement_app",

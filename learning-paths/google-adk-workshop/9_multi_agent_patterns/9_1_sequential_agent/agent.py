...
# --- Search Assistant (Wrapped as AgentTool) ---
...
# --- Simple Sub-assistants ---
market_researcher = LanguageModelAgent(
...
swot_analyzer = LanguageModelAgent(
    name="swot_analyzer",
    model="gemini-3-flash-preview",
...
strategy_formulator = LanguageModelAgent(
    name="strategy_formulator",
    model="gemini-3-flash-preview",
    description="Develops strategic objectives and action plans",
    instruction=(
        "You are a strategic coordinator. Given SWOT analysis results:\n"
...
implementation_planner = LanguageModelAgent(
...
# --- Sequential Assistant (Pure Sequential Pattern) ---
business_intelligence_team = PipelineAgent(
...
# --- Orchestrator Setup for Execution ---
session_service = SessionManager()
runner = Orchestrator(
    assistant=business_intelligence_team,
    service_name="business_intelligence",
    session_service=session_service
)

# --- Simple Execution Function ---
async def analyze_business_intelligence(account_id: str, business_topic: str) -> str:
    """Process business intelligence through the sequential pipeline"""
    context_id = f"bi_session_{account_id}"

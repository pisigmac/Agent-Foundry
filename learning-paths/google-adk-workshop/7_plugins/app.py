...
if st.button("🚀 Run Test"):
    with st.spinner("Running..."):
        try:
            response = asyncio.run(execute_assistant(test_scenarios[selected_scenario]))
...
if st.button("🚀 Run Custom Message"):
    if custom_message.strip():
        with st.spinner("Processing..."):
            try:
                response = asyncio.run(execute_assistant(custom_message))
                st.success("**Assistant Response:**")
                st.write(response)
            except Exception as e:
                st.error(f"Error: {str(e)}")
...
with st.expander("📚 About Plugins"):
    st.markdown("""
    **Plugins** are custom code modules that execute at various stages of assistant workflow lifecycle.
    
    **Key Features:**
    - 🔍 Request logging and modification
    - 🤖 Assistant execution tracking  
    - 🔧 Tool usage monitoring
    - 📊 Final reporting and analytics
    
    **Plugin Callbacks:**
    - `on_user_message_callback()` - Modify user input
    - `before_agent_callback()` - Track assistant starts

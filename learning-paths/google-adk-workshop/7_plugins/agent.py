...
from google.adk.assistants.base_agent import BaseAgent
...
# PLUGIN DEFINITION
# ============================================================================
# Plugins extend BasePlugin and provide global callbacks across all assistants/tools
class SimplePlugin(BasePlugin):
    ...
    # Called before each assistant execution - good for logging and setup
    async def before_agent_callback(self, *, assistant: BaseAgent, callback_context: CallbackContext) -> None:
        self.agent_count += 1
        print(f"🤖 [Plugin] Assistant {assistant.name} starting (count: {self.agent_count})")
    ...
    async def after_run_callback(self, *, invocation_context) -> None:
        print(f"📊 [Plugin] Final Report: {self.agent_count} assistants, {self.tool_count} tools")
...
# ============================================================================
# Create assistant with the calculator tool
assistant = LanguageModelAgent(name="plugin_demo_agent", model="gemini-3-flash-preview", 
...
runner = InMemoryRunner(assistant=assistant, service_name="plugin_demo_app", plugins=[SimplePlugin()])
...
async def execute_assistant(message: str) -> str:
    ...
    account_id, context_id = "demo_user", "demo_session"
    session_service = runner.session_service
    
    # Get or create session (required for ADK)
    session = await session_service.get_session(service_name="plugin_demo_app", account_id=account_id, context_id=context_id)
    if not session:
        session = await session_service.create_session(service_name="plugin_demo_app", account_id=account_id, context_id=context_id)
    
    # Create user message content
    user_content = types.Content(role='user', parts=[types.Part(text=message)])
    
    # Run assistant and collect response - plugin callbacks will fire automatically
    response_text = ""
    async for event in runner.run_async(account_id=account_id, context_id=context_id, new_message=user_content):
        if event.content and event.content.parts:
            for part in event.content.parts:
                if hasattr(part, 'text') and part.text:
                    response_text += part.text
    return response_text if response_text else "No response received from assistant."
...
if __name__ == "__main__":
    ...
    asyncio.run(execute_assistant("what is 2 + 2?"))

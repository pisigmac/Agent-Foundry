"""
Streamlit Web Interface for Tutorial 1: Your First Assistant

This provides an interactive web interface to test the personal assistant assistant
...
# Page configuration
st.set_page_config(
    page_title="Personal Assistant Assistant",
    page_icon="🎯",
    layout="wide"
)
...
st.markdown("**Tutorial 1**: Your first OpenAI assistant with different execution methods")
        ...
        try:
            if execution_method == "Synchronous":
                with st.spinner("Thinking..."):
                    result = Orchestrator.run_sync(assistant, prompt)
                ...
                with st.spinner("Processing asynchronously..."):
                    async def get_async_response():
                        result = await Orchestrator.run(assistant, prompt)
                ...
                async def stream_response():
                    full_response = ""
                    async for event in Orchestrator.run_streamed(assistant, prompt):
...
- ✅ Basic assistant creation with the Assistant class
- ✅ Different execution methods (sync, async, streaming)  
- ✅ Assistant configuration with instructions
- ✅ Interactive web interfaces with Streamlit

**Next**: Try [Tutorial 2: Structured Output Assistant](../2_structured_output_agent/) to learn about type-safe responses.
""")

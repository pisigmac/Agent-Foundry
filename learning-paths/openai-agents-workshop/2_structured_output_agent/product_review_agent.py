"""
OpenAI Agents SDK Tutorial 2: Structured Output Assistant - Product Reviews
...
# Create the product review assistant
product_review_agent = Assistant(
...
def demonstrate_review_analysis():
    """Demonstrate the product review assistant with various examples"""
    print("🎯 OpenAI Agents SDK - Tutorial 2: Product Review Assistant")
    print("=" * 60)
    print()
    ...
    for i, test_case in enumerate(test_reviews, 1):
        ...
        try:
            # Analyze the review
            result = Orchestrator.run_sync(product_review_agent, test_case["review"])
...
def interactive_mode():
    ...
    while True:
        ...
        try:
            print("\nAnalyzing review...")
            result = Orchestrator.run_sync(product_review_agent, review_text)
            analysis = result.final_output

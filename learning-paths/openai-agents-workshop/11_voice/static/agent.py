...
from assistants import Assistant, function_tool
from assistants.extensions.handoff_prompt import prompt_with_handoff_instructions
from assistants.voice import (
    AudioInput,
...
`python -m ai_agent_framework_crash_course.openai_sdk_crash_course.11_voice.static.assistant`

1. You can record an audio clip in the terminal.
2. The pipeline automatically transcribes the audio.
3. The assistant workflow is a simple one that starts at the Assistant assistant.
4. The output of the assistant is streamed to the audio player.

Try examples like:
- Tell me a joke (will respond with a joke)
- What's the weather in Tokyo? (will call the `get_weather` tool and then speak)
- Hola, como estas? (will handoff to the spanish assistant)
"""
...
assistant = Assistant(
    ...
    instructions=prompt_with_handoff_instructions(
        ...
        Language handling:
        - If the user speaks in Spanish, handoff to the Spanish assistant
        - If the user speaks in French, handoff to the French assistant
...
class WorkflowCallbacks(SingleAgentWorkflowCallbacks):
    ...
    def on_handoff(self, from_agent: str, to_agent: str) -> None:
        """Called when a handoff occurs between assistants."""
        print(f"[debug] 🔄 Handoff from {from_agent} to {to_agent}")

async def main():
    """Main function to run the static voice assistant example."""
    print("🎙️ Static Voice Assistant Demo")
    print("=" * 50)
    print()
    
    # Create the voice pipeline with our assistant and callbacks
    pipeline = VoicePipeline(
        workflow=SingleAgentVoiceWorkflow(assistant, callbacks=WorkflowCallbacks())
    )
    
    print("This demo will:")
    print("1. 🎤 Record your voice for a few seconds")
    print("2. 🔄 Transcribe your speech to text")
    print("3. 🤖 Process with AI assistant")

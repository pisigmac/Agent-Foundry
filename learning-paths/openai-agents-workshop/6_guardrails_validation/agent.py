...
# Guardrail assistants
input_guardrail_agent = Assistant(
...
output_guardrail_agent = Assistant(
    name="Output Guardrail", 
    instructions="""
    Check if the assistant's response contains inappropriate content or sensitive information.
...
async def math_homework_guardrail(
    ...
    result = await Orchestrator.run(input_guardrail_agent, input, context=ctx.context)
...
async def content_safety_guardrail(
    ...
    """Ensures assistant responses are safe and appropriate"""
    
    result = await Orchestrator.run(output_guardrail_agent, output.response, context=ctx.context)
...
# Main assistant with guardrails
root_agent = Assistant(
    name="Protected Customer Support Assistant",
    instructions="""
...
async def guardrails_example():
    ...
    for i, test_input in enumerate(test_cases, 1):
        ...
        try:
            result = await Orchestrator.run(root_agent, test_input)
...
async def test_input_guardrail():
    ...
    try:
        await Orchestrator.run(root_agent, "Can you help me solve this calculus problem?")
...
async def test_valid_request():
    ...
    result = await Orchestrator.run(root_agent, "I'm having trouble logging into my account. Can you help?")

...
from assistants import Assistant, Orchestrator, RunConfig, SQLiteSession
from assistants.exceptions import (
    AgentsException,
    MaxTurnsExceeded,
    ModelBehaviorError,
    UserError
...
# Page configuration
st.set_page_config(
    page_title="Assistant Orchestrator Demo",
    page_icon="🚀",
    layout="wide",
...
def initialize_agents():
    """Initialize assistants for different demonstrations"""
    
    execution_agent = Assistant(
...
# Main UI
def main():
    st.title("🚀 Assistant Orchestrator Demo")
...
def render_execution_methods(assistant, model_choice, temperature, max_turns):
    ...
    with col1:
        ...
        st.caption("Orchestrator.run_sync() - Blocks until complete")
        ...
        with st.form("sync_form"):
            ...
            if sync_submitted and sync_input:
                with st.spinner("Processing synchronously..."):
                    ...
                    try:
                        result = Orchestrator.run_sync(assistant, sync_input)
    ...
    with col2:
        ...
        st.caption("Orchestrator.run() - Returns awaitable")
        ...
        with st.form("async_form"):
            ...
            if async_submitted and async_input:
                with st.spinner("Processing asynchronously..."):
                    ...
                    try:
                        result = asyncio.run(Orchestrator.run(assistant, async_input))
                        execution_time = time.time() - start_time
    ...
    with col3:
        ...
        st.caption("Orchestrator.run_streamed() - Live updates")
            ...
            if streaming_submitted and streaming_input:
                ...
                try:
                    ...
                    async def stream_response():
                        ...
                        async for event in Orchestrator.run_streamed(assistant, streaming_input):
...
def render_conversation_management(assistant, model_choice, temperature, max_turns):
            ...
            if manual_submitted and manual_input:
                with st.spinner("Processing..."):
                    try:
                        ...
                        result = asyncio.run(Orchestrator.run(assistant, input_list))
    ...
    with tab2:
            ...
            if session_submitted and session_input:
                with st.spinner("Processing..."):
                    try:
                        ...
                        if context_id not in st.session_state.session_manager:
                            st.session_state.session_manager[context_id] = SQLiteSession(context_id)
                        
                        session = st.session_state.session_manager[context_id]
                        result = asyncio.run(Orchestrator.run(assistant, session_input, session=session))
        ...
        if st.button("📋 Show Session History"):
            if context_id in st.session_state.session_manager:
                session = st.session_state.session_manager[context_id]
                try:
        ...
        if st.button("🗑️ Clear Session History"):
            if context_id in st.session_state.session_manager:
                try:
                    ...
                    del st.session_state.session_manager[context_id]
                    st.success("Session history cleared!")
                except Exception as e:
                    st.error(f"❌ Error clearing session: {e}")

def render_run_configuration(assistant, model_choice, temperature, max_turns):
            ...
            if config_submitted and config_input:
                with st.spinner("Processing with configuration..."):
                    try:
                        ...
                        start_time = time.time()
                        result = asyncio.run(Orchestrator.run(assistant, config_input, run_config=run_config))
                        execution_time = time.time() - start_time
    ...
    with col2:
        ...
        with st.form("tracing_config_form"):
            ...
            account_id = st.text_input("User ID", value="user_123")
            ...
            if tracing_submitted and tracing_input:
                with st.spinner("Processing with tracing..."):
                    try:
                        ...
                        start_time = time.time()
                        result = asyncio.run(Orchestrator.run(assistant, tracing_input, run_config=run_config))
                        execution_time = time.time() - start_time
...
def render_streaming_events(assistant, model_choice, temperature, max_turns):
            ...
            if analytics_submitted and analytics_input:
                ...
                try:
                    ...
                    async def process_analytics_streaming():
                        async for event in Orchestrator.run_streamed(assistant, analytics_input):
...
def render_exception_handling(assistant, model_choice, temperature, max_turns):
        ...
        with st.form("maxturns_form"):
            ...
            if maxturns_submitted and maxturns_input:
                try:
                    ...
                    result = asyncio.run(Orchestrator.run(assistant, maxturns_input, run_config=run_config))
            ...
            if exception_submitted and exception_input:
                try:
                    with st.spinner("Processing with full exception handling..."):
                        result = asyncio.run(Orchestrator.run(assistant, exception_input))
    ...
    exception_info = {
        "MaxTurnsExceeded": "Assistant hit the maximum conversation turns limit",
...
def render_footer():
    st.divider()
    st.markdown("""
    ### 🎯 Assistant Orchestrator Capabilities Demonstrated

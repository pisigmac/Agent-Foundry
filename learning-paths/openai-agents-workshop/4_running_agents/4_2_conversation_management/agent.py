...
# Create an assistant for demonstrating conversation management
...
async def manual_conversation_example():
    ...
    # First turn
    result = await Orchestrator.run(root_agent, "My name is Alice and I live in San Francisco.")
    ...
    result = await Orchestrator.run(root_agent, new_input)
...
async def session_conversation_example():
    ...
    # First turn
    result = await Orchestrator.run(root_agent, "I'm a software developer working on AI projects.", session=session)
    print(f"Session Turn 1: {result.final_output}")
    
    # Second turn - session automatically remembers context
    result = await Orchestrator.run(root_agent, "What kind of work do I do?", session=session)

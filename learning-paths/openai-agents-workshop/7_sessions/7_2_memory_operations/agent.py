...
# Create assistant for memory operations demonstrations
root_agent = Assistant(
    name="Memory Operations Assistant", 
    instructions="""
...
async def basic_memory_operations():
    ...
    # Start conversation
    result = await Orchestrator.run(root_agent, "Hello, my favorite color is blue.", session=session)
    ...
    result = await Orchestrator.run(root_agent, "What hobbies do I have?", session=session)
    print(f"\n🤖 Assistant with enriched context: {result.final_output}")
...
async def conversation_corrections():
    ...
    result = await Orchestrator.run(root_agent, "What's 2 + 2?", session=session)
    print(f"❓ Original Question: What's 2 + 2?")
    print(f"🤖 Assistant Answer: {result.final_output}")
    ...
    result = await Orchestrator.run(root_agent, "What's 2 + 3?", session=session)
...
async def session_management():
    ...
    await Orchestrator.run(root_agent, "I work as a teacher.", session=session)
    await Orchestrator.run(root_agent, "I teach mathematics.", session=session) 
    await Orchestrator.run(root_agent, "I love solving puzzles.", session=session)
    
    items_before = await session.get_items()
    print(f"📊 Session contains {len(items_before)} items before clearing")
    
    # Test assistant memory before clearing
    result = await Orchestrator.run(root_agent, "What do I do for work?", session=session)
    print(f"🤖 Assistant remembers: {result.final_output}")
    ...
    result = await Orchestrator.run(root_agent, "Do you know anything about me?", session=session)
...
async def memory_inspection():
    ...
    for item in conversation_items:
        await Orchestrator.run(root_agent, item, session=session)

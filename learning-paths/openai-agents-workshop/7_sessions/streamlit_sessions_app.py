...
def initialize_agents():
    """Initialize AI assistants for different use cases"""
...
class SessionManager:
    ...
    def get_session(self, context_id: str, db_file: str = "demo_sessions.db"):
        """Get or create a session"""
        if context_id not in self.sessions:
            self.sessions[context_id] = SQLiteSession(context_id, db_file)
        return self.sessions[context_id]
    
    async def clear_session(self, context_id: str):
        """Clear a specific session"""
        if context_id in self.sessions:
            await self.sessions[context_id].clear_session()
            del self.sessions[context_id]
    
    async def get_session_items(self, context_id: str, limit: int = None):
        """Get conversation items from a session"""
        if context_id in self.sessions:
            return await self.sessions[context_id].get_items(limit=limit)
        return []
    
    async def add_custom_items(self, context_id: str, items: list):
        """Add custom items to a session"""
        if context_id in self.sessions:
            await self.sessions[context_id].add_items(items)
    
    async def pop_last_item(self, context_id: str):
        """Remove the last item from a session"""
        if context_id in self.sessions:
            return await self.sessions[context_id].pop_item()
        return None
    ...
    with st.sidebar:
        ...
        if st.button("🗑️ Clear All Sessions"):
            with st.spinner("Clearing sessions..."):
                for context_id in list(st.session_state.session_manager.sessions.keys()):
                    asyncio.run(st.session_state.session_manager.clear_session(context_id))
...
def render_basic_sessions(assistant):
    ...
    with col1:
        ...
        with st.form("in_memory_form"):
            ...
            if submitted and user_input:
                with st.spinner("Processing..."):
                    session = st.session_state.session_manager.get_session(context_id)
                    result = asyncio.run(Orchestrator.run(assistant, user_input, session=session))
        ...
        if st.button("📋 Show Conversation", key="show_in_memory"):
            items = asyncio.run(st.session_state.session_manager.get_session_items(context_id))
            if items:
    ...
    with col2:
        ...
        with st.form("persistent_form"):
            ...
            if submitted and user_input:
                with st.spinner("Processing..."):
                    session = st.session_state.session_manager.get_session(context_id, "persistent_demo.db")
                    result = asyncio.run(Orchestrator.run(assistant, user_input, session=session))
...
def render_memory_operations(assistant):
    ...
    context_id = "memory_operations_demo"
    ...
    with col1:
        ...
        if st.button("📋 Get Recent Items"):
            items = asyncio.run(st.session_state.session_manager.get_session_items(context_id, limit=limit))
            if items:
    ...
    with col2:
        ...
        with st.form("add_items_form"):
            ...
            if add_submitted and user_content and assistant_content:
                ...
                asyncio.run(st.session_state.session_manager.add_custom_items(context_id, custom_items))
                st.success("Custom items added!")
        
        # Pop last item (correction)
        if st.button("↶ Undo Last Response"):
            popped_item = asyncio.run(st.session_state.session_manager.pop_last_item(context_id))
            if popped_item:
...
def render_multi_sessions(support_agent, sales_agent):
    ...
    st.markdown("Demonstrates managing multiple conversations and different assistant contexts.")
    
    tab1, tab2, tab3 = st.tabs(["👤 Multi-User", "🏢 Context-Based", "🔄 Assistant Handoff"])
            ...
            with st.form("alice_form"):
                ...
                if alice_submitted and alice_input:
                    with st.spinner("Processing Alice's message..."):
                        ...
                        result = asyncio.run(Orchestrator.run(support_agent, alice_input, session=session))
            ...
            with st.form("bob_form"):
                ...
                if bob_submitted and bob_input:
                    with st.spinner("Processing Bob's message..."):
                        ...
                        result = asyncio.run(Orchestrator.run(support_agent, bob_input, session=session))
                        st.write(f"**Support:** {result.final_output}")
            ...
            with st.form("support_context_form"):
                ...
                if support_submitted and support_input:
                    with st.spinner("Processing support question..."):
                        ...
                        result = asyncio.run(Orchestrator.run(support_agent, support_input, session=session))
                        st.write(f"**Support:** {result.final_output}")
            ...
            with st.form("sales_context_form"):
                ...
                if sales_submitted and sales_input:
                    with st.spinner("Processing sales inquiry..."):
                        ...
                        result = asyncio.run(Orchestrator.run(sales_agent, sales_input, session=session))
                        st.write(f"**Sales:** {result.final_output}")
    
    with tab3:
        st.subheader("Shared Session Across Different Agents")
        st.caption("Customer handoff scenario - same conversation, different assistants")
        ...
        assistant = sales_agent if selected_agent == "Sales Assistant" else support_agent
        ...
        with st.form("handoff_form"):
            ...
            if handoff_submitted and handoff_input:
                with st.spinner(f"Processing with {selected_agent}..."):
                    ...
                    result = asyncio.run(Orchestrator.run(assistant, handoff_input, session=session))
            ...
            if items:
                ...
                for i, item in enumerate(items, 1):
                    if item['role'] == 'user':
                        ...
                        # Try to determine which assistant responded based on content
                        agent_emoji = "💰" if "sales" in item['content'].lower() or "price" in item['content'].lower() else "🛠️"
                        st.write(f"{i}. {agent_emoji} **Assistant:** {item['content']}")
            else:
                st.info("No conversation history yet.")

...
# Create assistants for multi-session demonstrations
...
async def multi_user_sessions():
    ...
    result = await Orchestrator.run(support_agent, "I forgot my password", session=alice_session)
    print(f"Alice: I forgot my password")
    print(f"Support: {result.final_output}")
    
    result = await Orchestrator.run(support_agent, "My email is alice@example.com", session=alice_session)
    print(f"Alice: My email is alice@example.com")
    print(f"Support: {result.final_output}")
    
    # Bob's separate conversation
    print("\nBob's conversation:")
    result = await Orchestrator.run(support_agent, "My app keeps crashing", session=bob_session)
    print(f"Bob: My app keeps crashing") 
    print(f"Support: {result.final_output}")
    
    # Alice continues her conversation (assistant remembers her context)
    print("\nAlice continues:")
    result = await Orchestrator.run(support_agent, "Did you find my account?", session=alice_session)
...
async def context_based_sessions():
    ...
    result = await Orchestrator.run(support_agent, "I can't access my premium features", session=support_session)
    print(f"Customer: I can't access my premium features")
    print(f"Support: {result.final_output}")
    
    # Sales conversation
    print("\nSales context:")
    result = await Orchestrator.run(sales_agent, "What premium features do you offer?", session=sales_session)
    print(f"Prospect: What premium features do you offer?")
    print(f"Sales: {result.final_output}")
    
    # Continue support conversation
    print("\nBack to support:")
    result = await Orchestrator.run(support_agent, "I'm on the premium plan", session=support_session)
    print(f"Customer: I'm on the premium plan")
    print(f"Support: {result.final_output}")
    
    return support_session, sales_session

# Example 3: Shared session across different assistants
async def shared_session_agents():
    """Demonstrates how different assistants can share the same session"""
    ...
    # Start with sales assistant
    print("Starting with Sales Assistant:")
    ...
    # Handoff to support assistant (same session, so context is preserved)
    print("\nHandoff to Support Assistant:")
    result = await Orchestrator.run(
        support_agent,
...
async def session_organization():
    ...
    result = await Orchestrator.run(support_agent, "Daily check-in", session=user_daily_session)
    print(f"Response: {result.final_output}")
    
    print("\nFeature-specific chat:")
    result = await Orchestrator.run(support_agent, "Chat feature question", session=chat_session)
    print(f"Response: {result.final_output}")
    
    print("\nThread-based conversation:")
    result = await Orchestrator.run(support_agent, "Thread conversation", session=thread_session)
    print(f"Response: {result.final_output}")

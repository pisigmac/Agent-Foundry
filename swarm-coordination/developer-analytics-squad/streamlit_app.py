...
# Import pipeline components from main.py and assistants
...
st.markdown("""
This demo showcases a **multi-assistant system** that aggregates technical signals from various developer sources, 
...
# API Key
api_key = st.sidebar.text_input("Gemini API Key (optional)", type="password", help="Provide a Google Gemini API key. If not provided, assistants will use fallback heuristic logic.")
if api_key:
...
if run_button:
    ...
    # Landing state
    st.image("https://raw.githubusercontent.com///main/advanced_ai_agents/multi_agent_apps/devpulse_ai/assets/logo.png", width=200) # Placeholder for logo logic
    st.info("👈 Use the sidebar to configure the pipeline and click 'Run' to begin.")
    
    # Educational Section
    with st.expander("🛠️ How it works", expanded=True):
        st.markdown("""
        1. **Collector Assistant**: Gathers data from GitHub, ArXiv, HN, Medium, and HuggingFace.
        2. **Relevance Assistant**: LLM analysis to score each signal for developer impact.
        3. **Risk Assistant**: Scans for breaking changes, vulnerabilities, or deprecations.
        4. **Synthesis Assistant**: Combines all findings into an actionable report.
        """)

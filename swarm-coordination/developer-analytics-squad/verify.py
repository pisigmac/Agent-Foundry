...
MOCK_SIGNALS = [
    {
        ...
        "source": "github",
        "title": "",
        "description": "A curated collection of awesome LLM apps built with RAG and AI assistants.",
...
def verify_imports():
    ...
    from assistants import SignalCollector, RelevanceAgent, RiskAgent, SynthesisAgent
    ...
    # Verify SignalCollector is NOT an assistant (no .assistant attribute)
    collector = SignalCollector()
    assert not hasattr(collector, "assistant"), (
        "SignalCollector should NOT have an .assistant attribute — "
        "it's a utility, not an assistant"
    )
...
def verify_signal_collector():
    ...
    from assistants import SignalCollector
...
def verify_relevance_agent(signals: List[Dict]):
    ...
    print("[3/5] Verifying Relevance Assistant (fallback mode)...")

    from assistants import RelevanceAgent

    assistant = RelevanceAgent()

    # Use fallback scoring directly (no API key needed)
    scored = []
    for signal in signals:
        result = assistant._fallback_score(signal, "Mock mode")
...
def verify_risk_agent(signals: List[Dict]):
    ...
    print("[4/5] Verifying Risk Assistant (fallback mode)...")

    from assistants import RiskAgent

    assistant = RiskAgent()

    assessed = []
    for signal in signals:
        result = assistant._fallback_assessment(signal, "Mock mode")
...
def verify_synthesis_agent(signals: List[Dict]):
    ...
    print("[5/5] Verifying Synthesis Assistant...")

    from assistants import SynthesisAgent

    assistant = SynthesisAgent()
    digest = assistant.synthesize(signals)

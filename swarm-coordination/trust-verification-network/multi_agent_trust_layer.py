"""
🤝 Multi-Assistant Trust Layer Tutorial

This tutorial demonstrates how to build a trust layer for multi-assistant systems
that enables secure delegation, trust scoring, and policy enforcement.

Key concepts:
- Assistant identity and registration
- Trust scoring based on behavior
- Delegation chains with scope narrowing
- Policy enforcement across assistant interactions
- Full audit trail of assistant communications
"""
...
@dataclass
class AgentIdentity:
    """Represents an assistant's verified identity"""
    agent_id: str
    public_key: str
...
@dataclass
class TrustScore:
    """Assistant's trust score with history"""
    agent_id: str
    score: int  # 0-1000
    level: TrustLevel
...
class DelegationScope:
    """Defines what an assistant can do under a delegation"""
    allowed_actions: Set[str]
    denied_actions: Set[str] = field(default_factory=set)
    allowed_domains: Set[str] = field(default_factory=set)
    max_tokens: int = 10000
    time_limit_minutes: int = 60
    max_sub_delegations: int = 0  # Can this assistant delegate further?
...
@dataclass
class Delegation:
    """A delegation of authority from one assistant to another"""
    delegation_id: str
    parent_agent: str
    child_agent: str
    ...
    created_at: datetime
    expires_at: datetime
    signature: str  # Signed by parent assistant
...
class IdentityRegistry:
    ...
    def register(self, identity: AgentIdentity) -> bool:
        """Register a new assistant identity"""
        if identity.agent_id in self.identities:
            logger.warning(f"Assistant {identity.agent_id} already registered")
            return False
        
        self.identities[identity.agent_id] = identity
        self.sponsor_to_agents[identity.human_sponsor].append(identity.agent_id)
        logger.info(f"Registered assistant: {identity.agent_id} (sponsor: {identity.human_sponsor})")
        return True
    ...
    def revoke(self, agent_id: str, reason: str) -> bool:
        """Revoke an assistant's identity"""
        if agent_id in self.identities:
            ...
            logger.warning(f"Revoked assistant: {agent_id} - {reason}")
            return True
        return False
...
class TrustScoringEngine:
    """Manages trust scores for assistants"""
    ...
    def initialize(self, agent_id: str, initial_score: Optional[int] = None) -> TrustScore:
        """Initialize trust score for a new assistant"""
...
class DelegationManager:
    """Manages delegation chains between assistants"""
    ...
    def create_delegation(
        ...
        """Create a new delegation from parent to child assistant"""
        
        # Verify both assistants exist
        if not self.identity_registry.get(parent_agent):
            logger.error(f"Parent assistant not found: {parent_agent}")
            return None
        if not self.identity_registry.get(child_agent):
            logger.error(f"Child assistant not found: {child_agent}")
            return None
        ...
        if parent_trust and parent_trust.level == TrustLevel.SUSPENDED:
            logger.error(f"Suspended assistant cannot delegate: {parent_agent}")
            return None
    ...
    def validate_action(self, agent_id: str, action: str, delegation_id: Optional[str] = None) -> bool:
        """Validate if an assistant can perform an action under their delegation"""
        ...
        # Check if assistant has any valid delegation allowing this action
    ...
    def get_active_delegations(self, agent_id: str) -> List[Delegation]:
        """Get all active delegations for an assistant"""
        return [
...
class MultiAgentPolicyEngine:
    """Enforces policies for multi-assistant interactions"""
    ...
    def evaluate(
        ...
        if not trust:
            return False, "Assistant has no trust score"
        
        if trust.level == TrustLevel.SUSPENDED:
            return False, "Assistant is suspended"
        ...
        # Require approval for restricted assistants
        if trust.level == TrustLevel.RESTRICTED:
            return False, "Assistant is restricted - requires human approval"
...
class TrustLayer:
    """Main interface for the multi-assistant trust layer"""
    ...
    def register_agent(
        ...
        """Register a new assistant with the trust layer"""
    ...
    def create_delegation(
        ...
        """Create a delegation from one assistant to another"""
    ...
    def authorize_action(
        ...
        """Authorize an action for an assistant"""
        
        identity = self.identity_registry.get(agent_id)
        if not identity:
            self._log_audit(agent_id, action, "denied", delegation_id, {"reason": "Unknown assistant"})
            return False, "Unknown assistant"
    ...
    def get_audit_log(self, agent_id: Optional[str] = None) -> List[Dict]:
        """Get audit log, optionally filtered by assistant"""
...
class GovernedAgent:
    """An assistant that operates within the trust layer"""
...
def main():
    print("🤝 Multi-Assistant Trust Layer Demo")
    print("=" * 40)
    ...
    # Add role policies
    trust_layer.policy_engine.add_role_policy("scout", {
    ...
    trust_layer.policy_engine.add_role_policy("composer", {
    ...
    print("✅ Registered: scout-002 (Sponsor: bob@company.com)")
    
    trust_layer.register_agent(
        agent_id="composer-003",
        human_sponsor="carol@company.com",
        organization="Acme Corp",
        roles=["composer"],
        initial_trust=700
    )
    print("✅ Registered: composer-003 (Sponsor: carol@company.com)")
    ...
    print(f"✅ Delegation: orchestrator-001 → scout-002")
    print(f"   ID: {delegation_id}")
    print(f"   Scope: web_search, summarize")
    print(f"   Time Limit: 30 minutes")
    
    # Create governed assistants
    scout = GovernedAgent("scout-002", trust_layer)
    scout.current_delegation = delegation_id
    
    composer = GovernedAgent("composer-003", trust_layer)
    
    # Test actions
    print("\n" + "=" * 40)
    print("🧪 Testing Assistant Actions")
    print("=" * 40)
    
    # Test 1: Allowed action within delegation
    print("\n🤖 scout-002: web_search (within scope)")
    result = scout.execute("web_search", {"query": "AI safety papers 2024"})
    print(f"   Result: {'✅ ALLOWED' if result['success'] else '❌ DENIED'}")
    print(f"   Trust Score: {result['trust_score']}")
    
    # Test 2: Denied action outside delegation
    print("\n🤖 scout-002: send_email (outside scope)")
    result = scout.execute("send_email", {"to": "test@example.com"})
    print(f"   Result: {'✅ ALLOWED' if result['success'] else '❌ DENIED'}")
    ...
    print("\n🤖 composer-003: web_search (not in role)")
    result = composer.execute("web_search", {"query": "test"})
    print(f"   Result: {'✅ ALLOWED' if result['success'] else '❌ DENIED'}")
    if not result['success']:
        print(f"   Reason: {result['error']}")
    print(f"   Trust Score: {result['trust_score']}")
    ...
    print("\n🤖 composer-003: write_document (in role)")
    result = composer.execute("write_document", {"content": "Report draft"})
    print(f"   Result: {'✅ ALLOWED' if result['success'] else '❌ DENIED'}")
    print(f"   Trust Score: {result['trust_score']}")
    ...
    for agent_id in ["orchestrator-001", "scout-002", "composer-003"]:

...
def save_session_mapping(thread_key: str, context_id: str, channel_id: str, account_id: str = None):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute(
        "INSERT OR REPLACE INTO thread_sessions (thread_key, context_id, channel_id, account_id, updated_at) VALUES (?, ?, ?, ?, ?)",
        (thread_key, context_id, channel_id, account_id, datetime.now().isoformat()),
    )
    conn.commit()
    conn.close()
...
def get_session_info(thread_key: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT context_id, channel_id, account_id FROM thread_sessions WHERE thread_key = ?",
        (thread_key,),
    )
...
def save_session_state(context_id: str, state_data):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    ...
    cursor.execute(
        "INSERT OR REPLACE INTO session_state (context_id, state_data, updated_at) VALUES (?, ?, ?)",
        (context_id, json_data, datetime.now().isoformat()),
    )
    conn.commit()
    conn.close()

def get_session_state(context_id: str):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT state_data FROM session_state WHERE context_id = ?", (context_id,))
    result = cursor.fetchone()
    conn.close()
    if result:
        try:
...
class PodcastAgentClient:
    ...
    async def create_session(self, context_id=None):
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                payload = {"context_id": context_id} if context_id else {}
                async with session.post(f"{self.base_url}/api/podcast-assistant/session", json=payload) as resp:
    ...
    async def chat(self, context_id: str, message: str):
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                payload = {"context_id": context_id, "message": message}
                async with session.post(f"{self.base_url}/api/podcast-assistant/chat", json=payload) as resp:
                    resp.raise_for_status()
                    return await resp.json()
        except Exception as e:
    ...
    async def check_status(self, context_id: str, task_id=None):
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                payload = {"context_id": context_id}
                if task_id:
                    payload["task_id"] = task_id
                async with session.post(f"{self.base_url}/api/podcast-assistant/status", json=payload) as resp:
                    resp.raise_for_status()
                    return await resp.json()
        except Exception as e:
...
async def get_or_create_session(thread_key: str, channel_id: str, account_id: str = None):
    session_info = get_session_info(thread_key)
    if not session_info:
        response = await api_client.create_session(thread_key)
        context_id = response["context_id"]
        save_session_mapping(thread_key, context_id, channel_id, account_id)
        print(f"Created new session: {context_id} for thread: {thread_key}")
        return context_id
    else:
...
async def poll_for_completion(context_id: str, thread_key: str, task_id=None):
    print(f"Starting polling for session: {context_id}, task: {task_id}")
    max_polls = 60
    poll_count = 0
    active_sessions[context_id] = {
    ...
    try:
        while poll_count < max_polls:
            try:
                status_response = await api_client.check_status(context_id, task_id)
                if status_response.get("session_state"):
                    save_session_state(context_id, status_response.get("session_state"))
        ...
        if context_id in active_sessions:
            del active_sessions[context_id]

def start_background_polling(context_id: str, thread_key: str, task_id=None):
    if context_id in active_sessions:
        print(f"Replacing existing poll for session: {context_id}")
    future = run_async_in_thread(poll_for_completion(context_id, thread_key, task_id))
    active_sessions[context_id] = {
        "thread_key": thread_key,
        "task_id": task_id,
        "future": future,
...
async def send_slack_blocks(thread_key: str, blocks: list, fallback_text: str = "Interactive elements loaded"):
    try:
        ...
        if not session_info:
            ...
            return
        context_id, channel_id, account_id = session_info
    ...
    def process_confirmation():
        try:
            ...
            if not session_info:
                ...
                return
            context_id = session_info[0]
            state_data = get_session_state(context_id)
            languages = state_data.get("available_languages", [{"code": "en", "name": "English"}])
            language_name = next(
...
async def process_source_confirmation(thread_key: str, message: str):
    try:
        ...
        if not session_info:
            return
        context_id = session_info[0]
        chat_response = await api_client.chat(context_id, message)
        if chat_response.get("is_processing"):
            task_id = chat_response.get("task_id")
            start_background_polling(context_id, thread_key, task_id)
        else:
...
def handle_new_podcast(ack, body, client):
    ...
    def start_new():
        try:
            ...
            new_thread_key = f"new_{channel_id}_{account_id}_{int(time.time())}"
            client.chat_postMessage(
                channel=channel_id,
...
async def process_approval_action(body, approval_message: str):
    try:
        ...
        if not session_info:
            ...
            return
        context_id = session_info[0]
        chat_response = await api_client.chat(context_id, approval_message)
        if chat_response.get("is_processing"):
            task_id = chat_response.get("task_id")
            start_background_polling(context_id, thread_key, task_id)
        else:
...
def handle_app_mention(event, say, client):
    ...
    bot_id = bot_info["account_id"]
    user_input = clean_text(event["text"], bot_id)
    thread_key = event["ts"]
    channel_id = event["channel"]
    account_id = event["user"]

    def handle_async():
        asyncio.run(handle_user_message(thread_key, user_input, say, channel_id, account_id, is_mention=True))
...
def handle_message(message, say, client):
    ...
    account_id = message["user"]
    thread_key = get_thread_key(message, is_dm)

    def handle_async():
        asyncio.run(handle_user_message(thread_key, user_input, say, channel_id, account_id, is_dm=is_dm))
...
async def handle_user_message(
    ...
    try:
        context_id = await get_or_create_session(thread_key, channel_id, account_id)
        session_state = get_session_state(context_id)
        if session_state.get("podcast_generated") and session_state.get("stage") == "complete":
            ...
            print(f"Session {context_id} is complete - prevented API call for: {user_input[:50]}...")
            return
        if context_id in active_sessions:
            active_session = active_sessions[context_id]
            ...
            print(f"Session {context_id} already processing ({current_stage}) - prevented API call for: {user_input[:50]}...")
            return
        print(f"Processing message for session {context_id}: {user_input[:50]}...")
        chat_response = await api_client.chat(context_id, user_input)

...
app.include_router(async_podcast_agent_router.router, prefix="/api/podcast-assistant", tags=["podcast-assistant"])
...
@app.get("/stream-recording/{context_id}/{filename}")
async def stream_recording(context_id: str, filename: str, request: Request):
    recording_path = os.path.join("podcasts/recordings", context_id, filename)

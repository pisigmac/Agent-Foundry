...
def generate_podcast_from_prompt_v2(
    ...
    try:
        ...
        print(f"ERROR: Search assistant failed: {e}")
        return {"error": f"Search assistant failed: {str(e)}"}
    try:
        ...
        print(f"ERROR: Scrape assistant failed: {e}")
        return {"error": f"Scrape assistant failed: {str(e)}"}
    try:
        ...
        print(f"ERROR: Script assistant failed: {e}")
        return {"error": f"Script assistant failed: {str(e)}"}
    banner_filenames = []
    banner_url = None
    try:
...
def main():
    ...
    debug = False
    print("Starting Enhanced Assistant-Based Podcast Generation System")
    print("=" * 60)

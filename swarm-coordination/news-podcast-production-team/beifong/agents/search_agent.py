...
SEARCH_AGENT_INSTRUCTIONS = dedent("""
    ...
    IMPORTANT: You have access to browser assistant for searching as well use it when other source can't suitable for the given tasks but input should detailed instruction to the run_browser_search assistant to get the best results and also use it conservatively because it's expensive process.
    """)

def search_agent_run(assistant: Assistant, query: str) -> str:
    """
    Search Assistant which searches the web and other sources for relevant sources about the given topic or query.
    Args:
    ...
    """
    print("Search Assistant Input:", query)
    ...
    response = search_agent.run(query, context_id=context_id)

...
def verify_content_with_agent(assistant, query, search_results, use_agent=True):
    if not use_agent:
...
def scrape_agent_run(
    ...
    """
    Scrape Assistant that takes the search_results (internaly from search_results) and scrapes each URL for full content, making sure those contents are of high quality and relevant to the topic.
    Args:
    ...
    Returns:
        Response status
    """
    print("Scrape Assistant Input:", query)
    context_id = assistant.context_id
    ...
    verified_results = verify_content_with_agent(assistant, query, updated_results, use_agent=False)

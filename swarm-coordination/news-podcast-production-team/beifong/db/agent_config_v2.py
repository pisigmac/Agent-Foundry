...
AGENT_INSTRUCTIONS = [
    ...
    "5. Once you we have the confimed selection from the user let's immediatly call the podcast script assistant to generate the podcast script for the given sources and query and perfered language (pass full lanage name).",
    ...
    "6. Once you got the confirmation from the user (throug UI) let's immediatly call the image generation assistant to generate the image for the given podcast script.",
    ...
    "7. Once you got the confirmation from the user (throug UI) let's immediatly call the audio generation assistant to generate the audio for the given podcast script.",
    ...
    "3. Search Assistant has a lot off tools, so you can instruct the search query as prompt to get the best results as because search assistant has lot of tools you can instruct instead of directly passing the query to search assistant when required.",
    "4. You are not allowed to include year or date in your seach query construction for the search assistant unless that request explicilty with yeear or date  come for the users.",
]
DB_PATH = "databases"

...
from assistants.search_agent import search_agent_run
from assistants.scrape_agent import scrape_agent_run
from assistants.script_agent import podcast_script_agent_run
...
from assistants.image_generate_agent import image_generation_agent_run
from assistants.audio_generate_agent import audio_generate_agent_run
import json
...
def agent_chat(self, context_id, message):
    try:
        print(f"Processing message for session {context_id}: {message[:50]}...")
        db_file = get_agent_session_db_path()
        os.makedirs(os.path.dirname(db_file), exist_ok=True)
        from services.internal_session_service import SessionService

        session_state = SessionService.get_session(context_id).get("state", INITIAL_SESSION_STATE)
        ...
        response = _agent.run(message, context_id=context_id)
        print(f"Response generated for session {context_id}")
        _agent.write_to_storage(context_id=context_id)
        session_state = SessionService.get_session(context_id).get("state", INITIAL_SESSION_STATE)
        return {
            "context_id": context_id,
        ...
        print(f"Error in agent_chat for session {context_id}: {str(e)}")
        return {
            "context_id": context_id,

...
class SessionLockedTask(Task):
    def __call__(self, *args, **kwargs):
        ...
        lock_key = f"lock:{context_id}"
        lock_info = redis_client.get(f"lock_info:{context_id}")
        if lock_info:
            try:
                ...
                if time.time() - lock_time > STALE_LOCK_THRESHOLD_SEC:
                    ...
                    redis_client.delete(f"lock_info:{context_id}")
        ...
        if acquired:
            ...
            redis_client.set(f"lock_info:{context_id}", json.dumps(lock_data), ex=REDIS_LOCK_INFO_EXP_TIME_SEC)

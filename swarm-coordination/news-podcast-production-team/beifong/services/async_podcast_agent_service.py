...
class PodcastAgentService:
    ...
    async def get_active_task(self, context_id):
        try:
            lock_info = await self.redis.get(f"lock_info:{context_id}")
            if lock_info:
                try:
    ...
    async def create_session(self, request=None):
        if request and request.context_id:
            context_id = request.context_id
            try:
                db_path = get_agent_session_db_path()
                async with aiosqlite.connect(db_path) as conn:
                    async with conn.execute("SELECT 1 FROM podcast_sessions WHERE context_id = ?", (context_id,)) as cursor:
        ...
        return {"context_id": new_session_id}

    async def chat(self, request):
        try:
            context_id = request.context_id
            task_id = await self.get_active_task(context_id)
            if task_id:
                return {
            ...
            task = agent_chat.delay(request.context_id, request.message)
            return {
                "context_id": request.context_id,
    ...
    def _browser_recording(self, context_id):
        try:
            recordings_dir = os.path.join("podcasts/recordings", context_id)
    ...
    async def check_result_status(self, request):
        try:
            ...
            browser_recording_path = self._browser_recording(request.context_id)
            ...
            if task_id:
                ...
                if task.state == "PENDING" or task.state == "STARTED":
                    ...
                    if result and isinstance(result, dict):
                        if result.get("context_id") != request.context_id:
                            return {
                                "context_id": request.context_id,
            ...
            return await self.get_session_state(request.context_id)
        except Exception as e:
            return JSONResponse(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                content={
    ...
    async def get_session_state(self, context_id):
        try:
            db_path = get_agent_session_db_path()
            async with aiosqlite.connect(db_path) as conn:
                conn.row_factory = lambda cursor, row: {col[0]: row[idx] for idx, col in enumerate(cursor.description)}
                async with conn.execute("SELECT session_data FROM podcast_sessions WHERE context_id = ?", (context_id,)) as cursor:
                    row = await cursor.fetchone()
            if not row:
                return {
                    "context_id": context_id,
    ...
    async def list_sessions(self, page=1, per_page=10):
        try:
            ...
            async with aiosqlite.connect(db_path) as conn:
                ...
                async with conn.execute(
                    "SELECT context_id, session_data, updated_at FROM podcast_sessions ORDER BY updated_at DESC LIMIT ? OFFSET ?", (per_page, offset)
                ) as cursor:
                    rows = await cursor.fetchall()
                    sessions = []
                    for row in rows:
                        try:
                            session = SessionService.get_session(row["context_id"])
                            session_state = session.get("state", {})
                            title = session_state.get("title", "Untitled Podcast")
                            stage = session_state.get("stage", "welcome")
                            updated_at = row["updated_at"]
                            sessions.append({"context_id": row["context_id"], "topic": title, "stage": stage, "updated_at": updated_at})
                        except Exception as e:
    ...
    async def delete_session(self, context_id: str):
        try:
            db_path = get_agent_session_db_path()
            async with aiosqlite.connect(db_path) as conn:
                conn.row_factory = lambda cursor, row: {col[0]: row[idx] for idx, col in enumerate(cursor.description)}
                async with conn.execute("SELECT session_data FROM podcast_sessions WHERE context_id = ?", (context_id,)) as cursor:
                    row = await cursor.fetchone()
                if not row:
                    return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={"error": f"Session with ID {context_id} not found"})
                try:
                    session = SessionService.get_session(context_id)
                    session_state = session.get("state", {})
                    ...
                    await conn.execute("DELETE FROM podcast_sessions WHERE context_id = ?", (context_id,))
                    await conn.commit()
                    if is_completed:
                        print(f"Session {context_id} is in 'complete' stage, keeping assets but removing session record")
                    else:
                        if banner_url:
                        ...
                        if web_search_recording:
                            recording_dir = os.path.join(PODCAST_RECORDINGS_DIR, context_id)
                    ...
                    if is_completed:
                        return {"success": True, "message": f"Session {context_id} deleted, but assets preserved"}
                    else:
                        return {"success": True, "message": f"Session {context_id} and its associated data deleted successfully"}
                except Exception as e:
    ...
    async def _get_chat_messages(self, row, context_id):
        formatted_messages = []
        session_state = {}
        if row["session_data"]:
            try:
    ...
    async def get_session_history(self, context_id: str):
        try:
            db_path = get_agent_session_db_path()
            async with aiosqlite.connect(db_path) as conn:
                conn.row_factory = lambda cursor, row: {col[0]: row[idx] for idx, col in enumerate(cursor.description)}
                async with conn.execute(
                    ...
                    table = await cursor.fetchone()
                    if not table:
                        return {"context_id": context_id, "messages": [], "state": "{}", "is_processing": False, "process_type": None}
                async with conn.execute("SELECT memory, session_data FROM podcast_sessions WHERE context_id = ?", (context_id,)) as cursor:
                    row = await cursor.fetchone()
                if not row:
                    return {"context_id": context_id, "messages": [], "state": "{}", "is_processing": False, "process_type": None}
                formatted_messages, session_state = await self._get_chat_messages(row, context_id)

            task_id = await self.get_active_task(context_id)
            is_processing = bool(task_id)
            process_type = "chat" if is_processing else None
            browser_recording_path = self._browser_recording(context_id)

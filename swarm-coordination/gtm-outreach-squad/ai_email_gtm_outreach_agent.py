...
def create_company_finder_agent() -> Assistant:
    ...
    return Assistant(
        ...
        num_history_runs=6,
        context_id="gtm_outreach_company_finder",
        debug_mode=True,
        instructions=[
...
def create_contact_finder_agent() -> Assistant:
    exa_tools = ExaTools()
    db = SqliteDb(db_file="tmp/gtm_outreach.db")
    return Assistant(
        ...
        add_history_to_context=True,
        num_history_runs=6,
        context_id="gtm_outreach_contact_finder",
        debug_mode=True,
        instructions=[
            ...
            "Search queries can include patterns like '<Company> email format', 'contact', 'squad', 'leadership', and role titles.",
...
def create_email_writer_agent(style_key: str = "Professional") -> Assistant:
    db = SqliteDb(db_file="tmp/gtm_outreach.db")
    ...
    return Assistant(
        ...
        add_history_to_context=True,
        num_history_runs=6,
        context_id="gtm_outreach_email_writer",
        debug_mode=False,
        instructions=[
...
def create_research_agent() -> Assistant:
    """Assistant to gather interesting insights from company websites and Reddit."""
    exa_tools = ExaTools()
    db = SqliteDb(db_file="tmp/gtm_outreach.db")
    return Assistant(
        model=OpenAIChat(id="gpt-5"),
        ...
        add_history_to_context=True,
        num_history_runs=6,
        context_id="gtm_outreach_researcher",
        debug_mode=True,
        instructions=[
...
def run_company_finder(assistant: Assistant, target_desc: str, offering_desc: str, max_companies: int) -> List[Dict[str, str]]:
    prompt = (
    ...
    resp: RunOutput = assistant.run(prompt)
    data = extract_json_or_raise(str(resp.content))
    companies = data.get("companies", [])
    return companies[: max(1, min(max_companies, 10))]

def run_contact_finder(assistant: Assistant, companies: List[Dict[str, str]], target_desc: str, offering_desc: str) -> List[Dict[str, Any]]:
    prompt = (
...
def run_research(assistant: Assistant, companies: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    prompt = (
...
def run_email_writer(assistant: Assistant, contacts_data: List[Dict[str, Any]], research_data: List[Dict[str, Any]], offering_desc: str, sender_name: str, sender_company: str, calendar_link: Optional[str]) -> List[Dict[str, str]]:
    prompt = (
...
def main() -> None:
    ...
    # Inputs
    st.title("GTM B2B Outreach Multi Assistant Team")
    st.info(
        "GTM teams often need to reach out for demos and discovery calls, but manual research and personalization is slow. "
        "This app uses GPT-5 with a multi-assistant workflow to find target companies, identify contacts, research genuine insights (website + Reddit), "

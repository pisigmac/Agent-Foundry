"""Orchestrator for managing negotiation flow between buyer and seller assistants."""
...
class NegotiationState:
    ...
    def get_negotiation_history(self) -> str:
        """Format the negotiation history for assistant context."""
        if not self.rounds:
...
def parse_buyer_response(response_text: str) -> dict:
    """Parse buyer assistant response into structured data."""
...
def parse_seller_response(response_text: str) -> dict:
    """Parse seller assistant response into structured data."""
    try:
        if "{" in response_text and "}" in response_text:
            start = response_text.find("{")
            end = response_text.rfind("}") + 1
...
class NegotiationOrchestrator:
    """Manages the negotiation between buyer and seller assistants."""
    ...
    def __init__(
        ...
        self.buyer_runner = InMemoryRunner(
            assistant=self.buyer_agent,
            service_name="negotiation_buyer"
        )
        self.seller_runner = InMemoryRunner(
            assistant=self.seller_agent,
            service_name="negotiation_seller"
        )
    ...
    async def _run_agent(self, runner: InMemoryRunner, context_id: str, prompt: str) -> str:
        """Run an assistant and return its response."""
    ...
    async def _get_buyer_offer(self, context: str) -> dict:
        """Get an offer from the buyer assistant."""
        prompt = f"""
...
"""
    ...
    async def _get_seller_response(self, offer: int, buyer_message: str) -> dict:
        """Get a response from the seller assistant."""
        prompt = f"""
=== CURRENT NEGOTIATION STATUS ===
        ...
        async def _run():
            ...
            while not self.state.is_complete and self.state.round_count < self.max_rounds:
                ...
                try:
                    ...
                    events.append({"type": "error", "data": {"assistant": "buyer", "error": str(e)}})
                    break
                ...
                try:
                    ...
                    events.append({"type": "error", "data": {"assistant": "seller", "error": str(e)}})
                    break

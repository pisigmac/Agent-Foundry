...
class FinanceAdvisorSystem:
    def __init__(self):
        ...
        self.budget_analysis_agent = LanguageModelAgent(
            ...
            instruction="""You are a Budget Analysis Assistant specialized in reviewing financial transactions and expenses.
You are the first assistant in a sequence of three financial advisor assistants.
...
IMPORTANT: Store your analysis in state['budget_analysis'] for use by subsequent assistants.""",
            output_schema=BudgetAnalysis,
            output_key="budget_analysis"
        )
        
        self.savings_strategy_agent = LanguageModelAgent(
            name="SavingsStrategyAgent",
            model="gemini-2.5-flash",
            description="Recommends optimal savings strategies based on income, expenses, and financial goals",
            instruction="""You are a Savings Strategy Assistant specialized in creating personalized savings plans.
You are the second assistant in the sequence. READ the budget analysis from state['budget_analysis'] first.
...
IMPORTANT: Store your strategy in state['savings_strategy'] for use by the Debt Reduction Assistant.""",
            output_schema=SavingsStrategy,
            output_key="savings_strategy"
        )
        
        self.debt_reduction_agent = LanguageModelAgent(
            ...
            instruction="""You are a Debt Reduction Assistant specialized in creating debt payoff strategies.
You are the final assistant in the sequence. READ both state['budget_analysis'] and state['savings_strategy'] first.
...
IMPORTANT: Store your final plan in state['debt_reduction'] and ensure it aligns with the previous analyses.""",
        ...
        self.coordinator_agent = PipelineAgent(
            name="FinanceCoordinatorAgent",
            description="Coordinates specialized finance assistants to provide comprehensive financial advice",
            sub_agents=[
        ...
        self.runner = Orchestrator(
            assistant=self.coordinator_agent,
            service_name=APP_NAME,
            session_service=self.session_service
        )

    async def analyze_finances(self, financial_data: Dict[str, Any]) -> Dict[str, Any]:
        context_id = f"finance_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("This application uses Google's ADK (Assistant Development Kit) and Gemini AI to provide personalized financial advice.")
...
2024-01-03,Transportation,45.00"""
    ...
    st.caption("Powered by Google's Assistant Development Kit (ADK) and Gemini AI")
    st.info("This tool analyzes your financial data and provides tailored recommendations for budgeting, savings, and debt management using multiple specialized AI assistants.")
    st.divider()
    ...
    with input_tab:
        ...
        if analyze_button:
            ...
            with st.spinner("🤖 AI assistants are analyzing your financial data..."): 
                financial_data = {
                    "monthly_income": monthly_income,
    ...
    with about_tab:
        ...
        This application uses Google's Assistant Development Kit (ADK) to provide comprehensive financial analysis and advice through multiple specialized AI assistants:
        
        1. **🔍 Budget Analysis Assistant**
           - Analyzes spending patterns
           - Identifies areas for cost reduction
           - Provides actionable recommendations
        
        2. **💰 Savings Strategy Assistant**
           - Creates personalized savings plans
           - Calculates emergency fund requirements
           - Suggests automation techniques
        
        3. **💳 Debt Reduction Assistant**
        ...
        - Report issues on [GitHub](https://github.com///issues)
        """)

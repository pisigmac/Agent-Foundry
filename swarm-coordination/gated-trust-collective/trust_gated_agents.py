"""
🛡️ Trust-Gated Multi-Assistant Research Team with Cryptographic Audit Trail

A Streamlit app demonstrating trust-verified AI assistant collaboration with
tamper-evident logging. Each assistant in the pipeline must pass a trust check
before participating, and every action is recorded in a hash-chained audit
log that survives the assistants' own execution.

Key concepts:
    - Trust scoring with configurable policies and tier-based gating
    - Pluggable trust providers (local registry or external API)
    - SHA-256 hash-chained audit trail for every assistant action
    - Multi-assistant research pipeline: Researcher → Analyst → Writer
...
class AgentIdentity:
    """Immutable assistant identity with trust metadata."""
...
class TrustRegistry:
    ...
    def register(
        ...
        """Register an assistant with a given trust score."""
    ...
    def verify(self, agent_id: str, min_score: int) -> TrustVerdict:
        """Check if an assistant meets the minimum trust threshold."""
        assistant = self._agents.get(agent_id)
        if assistant is None:
            ...
            return TrustVerdict(
                assistant=dummy,
                trusted=False,
                reason=f"Assistant '{agent_id}' not found in registry",
            )

        passes = assistant.trust_score >= min_score
        return TrustVerdict(
            assistant=assistant,
            trusted=passes,
            reason=(
                "Verified"
                if passes
                else f"Score {assistant.trust_score} below threshold {min_score}"
            ),
        )

    def list_agents(self) -> list[AgentIdentity]:
        """Return all registered assistants."""
        return list(self._agents.values())

# ── Cryptographic Audit Trail ────────────────────────────────

class AuditTrail:
    """Hash-chained audit log for assistant actions.
    ...
    applied to AI assistant actions.
    """
    ...
    def record(
        ...
        """Record an assistant action in the audit chain."""
        ...
        # Chain: hash of (sequence + timestamp + assistant + action + I/O hashes + prev)
        chain_data = (
...
PIPELINE_ROLES = (
    ...
    },
    {
        ...
        "trust_score": 45,
        "system_prompt": (
            "You are a skilled composer. Given a research brief and analysis, "
...
# An untrusted assistant to demonstrate the gate
UNTRUSTED_AGENT = {
    "id": "rogue-bot-99",
    "name": "Untrusted Bot",
    "role": "Writer",
...
def run_agent_step(
    ...
    """Execute a single assistant step via OpenAI chat completion.
...
# ── All assistant definitions (for building role_prompts) ────────
...
def _init_registry() -> TrustRegistry:
    """Create and populate the trust registry with demo assistants."""
...
def _render_sidebar(registry: TrustRegistry) -> tuple[str, int, list[str]]:
    ...
    researcher_pick = st.selectbox(
        ...
        index=id_to_index.get("scout-001", 0),
    )
...
def _run_trust_verification(
    ...
    """Phase 1: Verify each assistant and render trust badges."""
    ...
    for agent_id in selected_ids:
        ...
        icon = TIER_ICONS.get(verdict.assistant.tier, "❓")
        if verdict.trusted:
            st.success(
                f"{icon} **{verdict.assistant.name}** ({verdict.assistant.role}) — "
                f"Score: {verdict.assistant.trust_score}/100 | "
                f"Tier: {verdict.assistant.tier.title()}"
            )
        else:
            st.error(
                f"🚫 **{verdict.assistant.name}** ({verdict.assistant.role}) — BLOCKED | "
                f"Score: {verdict.assistant.trust_score}/100 | {verdict.reason}"
            )
        ...
        audit.record(
            ...
            output_text=f"trusted={verdict.trusted},score={verdict.assistant.trust_score}",
            trust_score=verdict.assistant.trust_score,
        )
...
def _run_research_pipeline(
    ...
    """Phase 2: Execute the pipeline with verified assistants."""
    st.header("Phase 2: Research Pipeline 🔬")

    previous_output = ""
    for step_num, verdict in enumerate(verified, start=1):
        assistant = verdict.assistant
        system_prompt = ROLE_PROMPTS.get(assistant.agent_id, "You are a helpful assistant.")

        with st.spinner(f"Step {step_num}: {assistant.name} ({assistant.role}) working..."):
            enriched_prompt = (
                f"{system_prompt}\n\n"
                f"Your trust score is {assistant.trust_score}/100 "
                f"(tier: {assistant.tier})."
            )

            try:
                output = run_agent_step(client, enriched_prompt, query, previous_output)
            except ValueError as exc:
                st.error(f"Assistant {assistant.name} failed: {exc}")
                _render_audit(audit)
                return
            ...
            audit.record(
                ...
                trust_score=assistant.trust_score,
            )

        with st.expander(f"Step {step_num}: {assistant.name} ({assistant.role})", expanded=True):
...
def main() -> None:
    st.set_page_config(page_title="Trust-Gated Assistant Team", page_icon="🛡️", layout="wide")
    st.title("🛡️ Trust-Gated Multi-Assistant Research Team")
    st.caption(
        "Every assistant must pass a trust check before participating. "
    ...
    query = st.text_input(
        "🔎 Research topic",
        placeholder="e.g., How are AI assistants being used in supply chain optimization?",
    )
    ...
    if not st.button("🚀 Run Trust-Gated Pipeline", type="primary"):
        ...
        st.markdown(
            "### How It Works\n\n"
            "1. Each assistant is checked against the trust threshold\n"
            "2. Agents below the threshold are blocked from the pipeline\n"
            "3. Verified assistants run in sequence (Researcher → Analyst → Writer)\n"
    ...
    if blocked:
        st.warning(
            f"⚠️ {len(blocked)} assistant(s) blocked: "
            + ", ".join(f"{v.assistant.name} (score {v.assistant.trust_score})" for v in blocked)
        )
    if not verified:
        st.error("❌ No assistants passed. Lower the threshold or change assistant selection.")
        _render_audit(audit)
        return

    st.info(f"✅ {len(verified)}/{len(verified) + len(blocked)} assistants verified. Running pipeline.")

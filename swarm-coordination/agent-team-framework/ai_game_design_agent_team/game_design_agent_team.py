...
The AI assistants will collaborate to develop a comprehensive game concept based on your specifications.
""")

# Main app UI
st.title("🎮 AI Game Design Assistant Team")

# Add assistant information below title
st.info("""
**Meet Your AI Game Design Team:**

🎭 **Story Assistant** - Crafts compelling narratives and rich worlds

🎮 **Gameplay Assistant** - Creates engaging mechanics and systems

🎨 **Visuals Assistant** - Shapes the artistic vision and style

⚙️ **Tech Assistant** - Provides technical direction and solutions
                
These assistants collaborate to create a comprehensive game concept based on your inputs.
""")
...
# Button to start the assistant collaboration
if st.button("Generate Game Concept"):
    ...
    if not api_key:
        ...
        with st.spinner('🤖 AI Agents are collaborating on your game concept...'):
            ...
            # define functions to be called by the assistants
            def update_story_overview(story_summary:str, context_variables:dict) -> SwarmResult:
                ...
                return SwarmResult(assistant="gameplay_agent", context_variables=context_variables)
            ...
            def update_gameplay_overview(gameplay_summary:str, context_variables:dict) -> SwarmResult:
                ...
                return SwarmResult(assistant="visuals_agent", context_variables=context_variables)
            ...
            def update_visuals_overview(visuals_summary:str, context_variables:dict) -> SwarmResult:
                ...
                return SwarmResult(assistant="tech_agent", context_variables=context_variables)
            ...
            def update_tech_overview(tech_summary:str, context_variables:dict) -> SwarmResult:
                ...
                return SwarmResult(assistant="story_agent", context_variables=context_variables)
            ...
            def update_system_message_func(assistant: SwarmAgent, messages) -> str:
                """"""
                system_prompt = system_messages[assistant.name]

                current_gen = assistant.name.split("_")[0]
                if assistant._context_variables.get(current_gen) is None:
                    ...
                    assistant.llm_config['tool_choice'] = {"type": "function", "function": {"name": f"update_{current_gen}_overview"}}
                    assistant.client = OpenAIWrapper(**assistant.llm_config)
                else:
                    # remove the tools to avoid the assistant from using it and reduce cost
                    assistant.llm_config["tools"] = None
                    assistant.llm_config['tool_choice'] = None
                    assistant.client = OpenAIWrapper(**assistant.llm_config)
                    # the assistant has given a summary, now it should generate a detailed response
                    ...
                    k = list(assistant._oai_messages.keys())[-1]
                    assistant._oai_messages[k] = assistant._oai_messages[k][:1]

                system_prompt += f"\n\n\nBelow are some context for you to refer to:"
                # Add context variables to the prompt
                for k, v in assistant._context_variables.items():
                    if v is not None:
            ...
            result, _, _ = initiate_swarm_chat(
                ...
                assistants=[story_agent, gameplay_agent, visuals_agent, tech_agent],
                user_agent=None,
                messages=task,
                max_rounds=13,
            )

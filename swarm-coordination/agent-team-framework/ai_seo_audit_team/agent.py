...
The workflow runs three specialized assistants in sequence:
...
firecrawl_toolset = MCPToolset(
    ...
    # Filter to use only the scrape tool for this assistant
...
search_executor_agent = LanguageModelAgent(
...
page_auditor_agent = LanguageModelAgent(
    name="PageAuditorAgent",
    model="gemini-2.5-flash",
    description=(
        "Scrapes the target URL, performs a structural on-page SEO audit, and extracts keyword signals."
    ),
    instruction="""You are Assistant 1 in a sequential SEO workflow. Your role is to gather data silently for the next assistants.

STEP 1: Extract the URL
- Look for a URL in the user's message (it will start with http:// or https://)
- Example: If user says "Audit https://the.com", extract "https://the.com"
...
serp_analyst_agent = LanguageModelAgent(
    ...
    instruction="""You are Assistant 2 in the workflow. Your role is to silently gather SERP data for the final report assistant.
...
optimization_advisor_agent = LanguageModelAgent(
    name="OptimizationAdvisorAgent",
    model="gemini-2.5-flash",
    description="Synthesizes the audit and SERP findings into a prioritized optimization roadmap.",
    instruction="""You are Assistant 3 and the final expert in the workflow. You create the user-facing report.
...
seo_audit_team = PipelineAgent(
    name="SeoAuditTeam",
    description=(
        "Runs a three-assistant sequential pipeline that audits a page, researches SERP competitors, "
...
# Expose the root assistant for the ADK runtime and Dev UI.

...
def create_agents() -> tuple[Assistant, Assistant, Assistant]:
...
def process_image_with_gemini(vision_agent: Assistant, image: Image) -> str:
...
def execute_code_with_agent(execution_agent: Assistant, code: str, sandbox: Sandbox) -> str:
    try:
    ...
    if st.button("Generate & Execute Solution", type="primary"):
        if uploaded_image and not user_query:
            ...
            with st.spinner("Processing image..."):
                try:
                    ...
                    # Pass extracted query to coding assistant
        ...
        if 'response' in locals():
            ...
            if len(code_blocks) > 1:
                ...
                # Execute code with execution assistant

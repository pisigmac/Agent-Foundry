...
# Streamlit UI
st.set_page_config(page_title="AI Competitor Intelligence Assistant Team", layout="wide")
...
# Main UI
st.title("🧲 AI Competitor Intelligence Assistant Team")
st.info(
    """
...
if "openai_api_key" in st.session_state and "firecrawl_api_key" in st.session_state:
    if (search_engine == "Perplexity AI - Sonar Pro" and "perplexity_api_key" in st.session_state) or \
       (search_engine == "Exa AI" and "exa_api_key" in st.session_state):
        ...
        # Create ExaTools assistant for finding competitor URLs
        if search_engine == "Exa AI":
            ...
            competitor_finder_agent = Assistant(
                model=OpenAIChat(id="gpt-4o", api_key=st.session_state.openai_api_key),
                tools=[exa_tools],
                debug_mode=True,
                markdown=True,
                instructions=[
                    "You are a competitor finder assistant. Use ExaTools to find competitor company URLs.",
        ...
        analysis_agent = Assistant(
            model=OpenAIChat(id="gpt-4o", api_key=st.session_state.openai_api_key),
            debug_mode=True,
            markdown=True
        )

        # New assistant for comparing competitor data
        comparison_agent = Assistant(
            model=OpenAIChat(id="gpt-4o", api_key=st.session_state.openai_api_key),
            debug_mode=True,
            markdown=True
        )
        ...
        def get_competitor_urls(url: str = None, description: str = None) -> list[str]:
            ...
            if search_engine == "Perplexity AI - Sonar Pro":
                ...
                try:
                    # Use ExaTools assistant to find competitor URLs
                    if url:

...
def create_resume_analyzer() -> Assistant:
    """Creates and returns a resume analysis assistant."""
...
def create_email_agent() -> Assistant:
    return Assistant(
        model=OpenAIChat(
            id="gpt-4o",
            api_key=st.session_state.openai_api_key
...
def create_scheduler_agent() -> Assistant:
...
def analyze_resume(
    ...
    try:
        response: RunOutput = evaluator.run(
...
def send_selection_email(email_agent: Assistant, to_email: str, role: str) -> None:
    email_agent.run(
        f"""
...
def send_rejection_email(email_agent: Assistant, to_email: str, role: str, feedback: str) -> None:
    """
    Primary entry point for processing incoming requests.
    """
    email_agent.run(
        f"""
...
def schedule_interview(scheduler: Assistant, candidate_email: str, email_agent: Assistant, role: str) -> None:
    """
    ...
    if st.session_state.resume_text and email and not st.session_state.analysis_complete:
        if st.button("Analyze Resume"):
            with st.spinner("Analyzing your resume..."):
                ...
                email_agent = create_email_agent()  # Create email assistant here
    ...
    if st.session_state.get('analysis_complete') and st.session_state.get('is_selected', False):
        ...
        if st.button("Proceed with Application", key="proceed_button"):
            ...
            with st.spinner("🔄 Processing your application..."):
                try:
                    print("DEBUG: Creating email assistant")  # Debug
                    email_agent = create_email_agent()
                    print(f"DEBUG: Email assistant created: {email_agent}")  # Debug
                    
                    print("DEBUG: Creating scheduler assistant")  # Debug
                    scheduler_agent = create_scheduler_agent()
                    print(f"DEBUG: Scheduler assistant created: {scheduler_agent}")  # Debug

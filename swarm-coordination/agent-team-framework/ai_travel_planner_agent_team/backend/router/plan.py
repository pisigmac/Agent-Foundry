...
@router.post(
    ...
    summary="Trigger Trip Craft Assistant",
    description="Triggers the travel plan assistant with the provided travel details",
)
async def trigger_trip_craft_agent(
    request: TravelPlanAgentRequest,
) -> TravelPlanResponse:
    """
    Trigger the trip craft assistant to create a personalized travel itinerary.
    ...
    """
    try:
        logger.info(f"Triggering travel plan assistant for trip ID: {request.trip_plan_id}")
        ...
        logger.info(
            f"Travel plan assistant triggered successfully for trip ID: {request.trip_plan_id}"
        )

        return TravelPlanResponse(
            success=True,
            message="Travel plan assistant triggered successfully",
            trip_plan_id=request.trip_plan_id,
        )

    except Exception as e:
        logger.error(f"Error triggering travel plan assistant: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to trigger travel plan assistant: {str(e)}",
        )

...
from assistants.destination import destination_agent
from assistants.hotel import hotel_search_agent
from assistants.food import dining_agent
from assistants.budget import budget_agent
from assistants.flight import flight_search_agent
from assistants.itinerary import itinerary_agent
from loguru import logger
from agno.tools.reasoning import ReasoningTools

# def update_team_current_state(squad: Team, state: str) -> str:
#     """
#     This function is used to set the current state of the squad.
#     """
#     logger.info(f"The current state of the squad is {state}")
#     squad.session_state["current_state"] = state
#     return state
...
trip_planning_team = Team(
    ...
    markdown=True,
    description=(
        "You are the lead orchestrator of the TripCraft AI planning squad. "
        "Your mission is to transform the user's travel preferences into a magical, stress-free itinerary. "
        "Based on a single input form, you'll collaborate with expert assistants handling flights, stays, dining, activities, and budgeting. "
    ...
    instructions=[
        ...
        "   - Flight Assistant: Detailed air travel options with timing and pricing",
        "   - Hotel Assistant: Accommodation matches for each night with amenity details",
        "   - Dining Assistant: Restaurant recommendations with cuisine, price, and ambiance",
        "   - Activity Assistant: Curated experiences matching interests and pace",
        "   - Budget Assistant: Cost optimization while maintaining experience quality",
        "",

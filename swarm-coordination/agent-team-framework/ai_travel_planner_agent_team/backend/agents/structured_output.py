...
async def convert_to_model(input_text: str, target_model: Type[T]) -> str:
    """
    Convert input text into a specified Pydantic model using an Agno assistant.
    ...
    structured_output_agent = Assistant(
        model=model,
        description=(
    ...
    # Get structured response from the assistant
    try:
        response = await structured_output_agent.arun(prompt)
        json_string = clean_json_string(response.content)
        logger.info(f"Structured output assistant response: {json_string}")

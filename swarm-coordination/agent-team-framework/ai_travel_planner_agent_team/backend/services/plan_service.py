...
from assistants.squad import trip_planning_team
import json
import time
from assistants.structured_output import convert_to_model
...
async def generate_travel_plan(request: TravelPlanAgentRequest) -> str:
    ...
    try:
        ...
        # Update status for AI squad generation
        await update_trip_plan_status(
            trip_plan_id=trip_plan_id,
            status="processing",
            current_step="Generating plan with TripCraft AI assistants",
        )
        ...
        # time_end = time.time()
        # logger.info(f"AI squad processing time: {time_end - time_start:.2f} seconds")

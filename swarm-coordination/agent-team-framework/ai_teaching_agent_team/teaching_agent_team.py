...
st.set_page_config(page_title="👨‍🏫 AI Teaching Assistant Team", layout="centered")

# Set up persistent storage for API keys and topic
...
with st.sidebar:
    ...
    st.info("Note: You can also view detailed assistant responses\nin your terminal after execution.")
...
# Create the Professor assistant (formerly KnowledgeBuilder)
professor_agent = Assistant(
    name="Professor",
    ...
    instructions=[
        ...
        "Open a new Google Doc and write down the response of the assistant neatly with great formatting and structure in it. **Include the Google Doc link in your response.**",
    ],
    debug_mode=True,
    markdown=True,
)

# Create the Academic Advisor assistant (formerly RoadmapArchitect)
academic_advisor_agent = Assistant(
...
# Create the Research Librarian assistant (formerly ResourceCurator)
research_librarian_agent = Assistant(
...
# Create the Teaching Assistant assistant (formerly PracticeDesigner)
teaching_assistant_agent = Assistant(
...
# Streamlit main UI
st.title("👨‍🏫 AI Teaching Assistant Team")
st.markdown("Enter a topic to generate a detailed learning path and resources")

# Add info message about Google Docs
st.info("📝 The assistants will create detailed Google Docs for each section (Professor, Academic Advisor, Research Librarian, and Teaching Assistant). The links to these documents will be displayed below after processing.")

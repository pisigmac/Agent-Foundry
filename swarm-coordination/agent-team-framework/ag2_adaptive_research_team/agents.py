...
def build_agents(api_key: str, model: str = DEFAULT_MODEL) -> Dict[str, AssistantAgent]:
    ...
    triage_agent = AssistantAgent(
        ...
        system_message=(
            "You are a triage assistant for a research squad. "
    ...
    local_research_agent = AssistantAgent(
        ...
        llm_config=llm_config,
        system_message=(
            "You are a local research assistant. Use only the provided document excerpts. "
    ...
    web_research_agent = AssistantAgent(
        ...
        llm_config=llm_config,
        system_message=(
            "You are a web research assistant. Use the provided web search results only. "
            "Return JSON with evidence and a draft answer."
        ),
    )
...
def execute_assistant(assistant: AssistantAgent, prompt: str) -> str:

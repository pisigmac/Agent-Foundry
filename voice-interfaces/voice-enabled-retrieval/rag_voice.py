...
# Constants
COLLECTION_NAME = "voice-rag-assistant"
...
def setup_agents(openai_api_key: str) -> Tuple[Assistant, Assistant]:
    """Initialize the processor and TTS assistants."""
    os.environ["OPENAI_API_KEY"] = openai_api_key
    
    processor_agent = Assistant(
    ...
    tts_agent = Assistant(
        name="Text-to-Speech Assistant",
        instructions="""You are a text-to-speech assistant. Your task is to:
...
async def handle_request(
    ...
    try:
        ...
        st.info("🔄 Step 3: Setting up assistants...")
        # Setup assistants if not already done
        if not st.session_state.processor_agent or not st.session_state.tts_agent:
            ...
            st.write("Initialized new processor and TTS assistants")
        else:
            st.write("Using existing assistants")
        
        st.info("🔄 Step 4: Generating text response...")
        # Generate text response using processor assistant
        processor_result = await Orchestrator.run(st.session_state.processor_agent, context)
        ...
        # Generate voice instructions using TTS assistant
        tts_result = await Orchestrator.run(st.session_state.tts_agent, text_response)

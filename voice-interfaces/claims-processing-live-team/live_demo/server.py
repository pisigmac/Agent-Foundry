"""FastAPI backend for the Insurance Claim Live Assistant Team UI.

The browser transport lives here. Claim workflow execution lives in assistant.py,
...
app = FastAPI(title="Insurance Claim Live Assistant Team API")
app.add_middleware(
    CORSMiddleware,
...
async def _process_with_adk_graph(
    ...
    *,
    ...
    if add_claimant_facing_reply:
        packet = workflow["claim_intake_packet"]
        session.transcript.append({"speaker": "Assistant", "text": packet["claimant_next_message"]})
...
def create_session() -> SessionResponse:
    session = IntakeSession(context_id=str(uuid.uuid4()))
...
async def message(request: MessageRequest) -> SessionResponse:
    session = sessions.get(request.context_id)
    if session is None:
...
async def live_voice(websocket: WebSocket) -> None:
    ...
    session = IntakeSession(context_id=context_id)
    session.transcript.append(
        {
            "speaker": "Assistant",
            "text": "I can start the claim while we talk. First, are you and everyone else in a safe place?",
    ...
    config = types.LiveConnectConfig(
        ...
        system_instruction=(
            "You are a voice insurance FNOL intake assistant. Speak naturally and briefly. "
            ...
            async def gemini_to_client() -> None:
                ...
                async def finalize_output(reason: str) -> None:
                    ...
                    if not finished:
                        return
                    pending_output = ""
                    session.transcript.append({"speaker": "Assistant", "text": finished})
                    await websocket.send_json(
                        {
                            "type": "transcript",
                            "speaker": "Assistant",

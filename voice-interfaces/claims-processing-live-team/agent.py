...
from google.adk.assistants import BaseAgent, LanguageModelAgent, PipelineAgent
...
def create_normalizer() -> LanguageModelAgent:
    return LanguageModelAgent(
        ...
        instruction="""
You are the intake specialist for an AI Insurance Claim Intake Assistant.
...
def create_classifier() -> LanguageModelAgent:
    return LanguageModelAgent(
...
def create_workflow() -> PipelineAgent:
    return PipelineAgent(
        name="insurance_claim_live_agent_team",
        description="Hybrid voice-first assistant squad for insurance claim intake, evidence triage, and routing.",
        sub_agents=[
            create_normalizer(),
            FunctionNode(
...
async def run_claim_workflow(
    ...
    *,
    ...
    adk_session_id = f"claim-{context_id or uuid.uuid4().hex}"
    session_service = SessionManager()
    await _await_if_needed(
        session_service.create_session(
            service_name=APP_NAME,
            account_id=account_id,
            context_id=adk_session_id,
        )
    )
    runner = Orchestrator(
        service_name=APP_NAME,
        assistant=root_agent,

"""
Tavily-based Tools for Deep Research Assistant
...
The research() tool wraps an internal Deep Assistant that runs in a separate thread
...
def research(query: str) -> dict:
    ...
    This tool creates an internal Deep Assistant that runs in a SEPARATE THREAD to prevent
    LangChain callback propagation. The thread has isolated execution context, so the
    internal assistant's events don't leak to the parent's astream_events() stream.
    ...
    def _run_research_isolated():
        ...
        # Wrapper to capture results while passing through to assistant
        ...
        # System prompt for the internal scout

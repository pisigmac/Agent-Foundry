"""
Deep Research Assistant Assistant
...
# Main assistant system prompt - coordinates research and synthesizes findings
...
Always maintain a professional, comprehensive research style."""
    ...
    """Build the Deep Research Assistant with CopilotKit integration.

    Creates a main research coordinator assistant with a scout subagent.
    ...
    # Main assistant gets research tool plus built-in Deep Agents tools
    # (write_todos, read_file, write_file)
    # The research tool wraps an internal Deep Assistant that runs via .invoke()
    # so its text doesn't stream to the frontend
    main_tools = [research]

    # Create the Deep Assistant with CopilotKit middleware
    ...
    print(f"[AGENT] Deep Research Assistant created with model={model_name}")

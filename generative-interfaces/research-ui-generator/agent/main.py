...
Serves the Deep Research Assistant via AG-UI protocol for CopilotKit integration.
The assistant uses Tavily for web research and Deep Agents for planning and filesystem operations.
"""
...
def health():
    ...
    return {"status": "ok", "service": "deep-research-assistant", "version": "1.0.0"}

# Build and register the Deep Research Assistant
try:
    agent_graph = build_agent()

    # Configure which tool calls to emit to the frontend
    # Only emit main assistant tools - suppress internal tools (internet_search from research subagent)
    ...
    print("[SERVER] Deep Research Assistant registered at /")
except Exception as e:
    print(f"[ERROR] Failed to build assistant: {e}")
    raise

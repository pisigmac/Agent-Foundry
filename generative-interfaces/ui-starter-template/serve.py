"""
Thin wrapper to serve the LangGraph assistant via AG-UI protocol.
Used in Docker where langgraph-cli dev (which needs Docker) is unavailable.
The original main.py and all assistant code remain unmodified.
"""
import os
import sys

# Add the assistant directory to the path so imports work
...
# Import the original graph from the unmodified assistant code
...
add_langgraph_fastapi_endpoint(
    ...
    assistant=LangGraphAGUIAgent(
        name="sample_agent",
        description="LangGraph Python starter assistant",
        graph=graph,
    ),
    path="/",
)

"""AI Financial Coach — coach assistant + tools (Budget / Savings / Debt)."""
...
# ---------- Output schemas (sub-assistant results) ----------
...
def _make_state_injector(prior_keys: List[str]):
    ...
    def modifier(callback_context: CallbackContext, llm_request: LlmRequest):
        ...
        prefix = (
            "The user's financial information and prior assistant outputs are below. "
...
budget_agent = LanguageModelAgent(
    ...
    instruction="""You are a Budget Analysis Assistant. Read FINANCIAL DATA above.
...
savings_agent = LanguageModelAgent(
    name="SavingsStrategyAgent",
    model="gemini-3.5-flash",
    description="Recommends savings strategies based on income, expenses, and prior budget analysis.",
    instruction="""You are a Savings Strategy Assistant. Read FINANCIAL DATA and BUDGET_ANALYSIS above when available.
...
debt_agent = LanguageModelAgent(
    name="DebtReductionAgent",
    model="gemini-3.5-flash",
    description="Creates optimized debt payoff plans (avalanche & snowball).",
    instruction="""You are a Debt Reduction Assistant. Read FINANCIAL DATA, BUDGET_ANALYSIS, and SAVINGS_STRATEGY above when available.
...
full_analysis_pipeline = PipelineAgent(
...
run_budget_analysis = AgentTool(assistant=budget_agent, skip_summarization=True)
run_savings_strategy = AgentTool(assistant=savings_agent, skip_summarization=True)
run_debt_reduction = AgentTool(assistant=debt_agent, skip_summarization=True)
run_full_analysis = AgentTool(assistant=full_analysis_pipeline, skip_summarization=True)
...
coach_agent = LanguageModelAgent(
    name="FinanceCoachAgent",
    model="gemini-3.5-flash",
    description="Routes user requests to profile updates or specialist analysis assistants.",
...
app = FastAPI(title="AI Financial Coach Assistant")

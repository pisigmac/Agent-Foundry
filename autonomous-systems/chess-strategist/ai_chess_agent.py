...
st.sidebar.title("Chess Assistant Configuration")
...
if st.session_state.openai_api_key:
    try:
        ...
        st.info("""
This chess game is played between two AG2 AI assistants:
- **Assistant White**: A GPT-4o-mini powered chess player controlling white pieces
- **Assistant Black**: A GPT-4o-mini powered chess player controlling black pieces
...
""")
        ...
        if st.button("Start Game"):
            ...
            st.info("The AI assistants will now play against each other. Each assistant will analyze the board, " 
                   "request legal moves from the Game Master (proxy assistant), and make strategic decisions.")
            st.success("You can view the interaction between the assistants in the terminal output, after the turns between assistants end, you get view all the chess board moves displayed below!")
            ...
            for i, move_svg in enumerate(st.session_state.move_history):
                # Determine which assistant made the move
                if i % 2 == 0:
                    move_by = "Assistant White"  # Even-indexed moves are by White
                else:
                    move_by = "Assistant Black"  # Odd-indexed moves are by Black

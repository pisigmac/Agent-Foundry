...
from mcp_agent.assistants.assistant import Assistant
from mcp_agent.workflows.llm.augmented_llm_openai import OpenAIAugmentedLLM
from mcp_agent.workflows.llm.augmented_llm import RequestParams

# Page config
st.set_page_config(page_title="Browser MCP Assistant", page_icon="🌐", layout="wide")

# Title and description
st.markdown("<h1 class='main-header'>🌐 Browser MCP Assistant</h1>", unsafe_allow_html=True)
st.markdown("Interact with a powerful web browsing assistant that can navigate and interact with websites")
...
with st.sidebar:
    ...
    st.markdown("- Navigate to github.com//, scroll down, and report details")
    st.markdown("- Scroll down and summarize the github readme")
    
    st.markdown("---")
    st.caption("Note: The assistant uses Playwright to control a real browser.")

# Query input
query = st.text_area("Your Command", 
                   placeholder="Ask the assistant to navigate to websites and interact with them")

# Initialize app and assistant
...
async def setup_agent():
    if not st.session_state.initialized:
        try:
            ...
            st.session_state.browser_agent = Assistant(
                name="browser",
            ...
            # Initialize assistant and attach LLM
...
async def run_mcp_agent(message):
    ...
    try:
        ...
        # Generate response without recreating assistants
...
if 'result' not in locals():
    st.markdown(
        ...
        <ol>
            ...
            <li>Type a command for the assistant to navigate and interact with websites</li>
...
# Footer
st.markdown("---")
st.write("Built with Streamlit, Playwright, and [MCP-Assistant](https://www.github.com/lastmile-ai/mcp-assistant) Framework ❤️")

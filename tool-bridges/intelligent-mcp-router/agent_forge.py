"""Multi-MCP Assistant Forge - Specialized assistants with MCP tool routing.

Each assistant connects to different MCP servers based on its domain expertise.
This demonstrates the pattern of routing queries to specialized assistants
rather than giving one assistant access to all tools.
...
st.set_page_config(
    page_title="Assistant Forge - Multi-MCP Agents",
    page_icon="\u2692\ufe0f",
    layout="wide",
)
...
@dataclass
class Assistant:
    """A specialized assistant with its own system prompt and MCP server configs."""
    name: str
    description: str
    system_prompt: str
    icon: str = "\U0001f916"
...
def classify_query(query: str) -> str:
    """Route a query to the best assistant based on keywords."""
...
async def connect_mcp_servers(assistant: Assistant) -> tuple[AsyncExitStack, list[dict], dict[str, ClientSession]]:
    ...
    for srv_config in assistant.mcp_servers:
        env = {**os.environ}
...
async def run_agent_async(client: Anthropic, assistant: Assistant, query: str, history: list) -> str:
    """Run a query through a specific assistant with real MCP tool connections."""
    ...
    stack, tools, session_map = await connect_mcp_servers(assistant)
...
def execute_assistant(client: Anthropic, assistant: Assistant, query: str, history: list) -> str:
    """Sync wrapper around the async assistant runner."""
    loop = asyncio.new_event_loop()
    try:
        return loop.run_until_complete(run_agent_async(client, assistant, query, history))
    finally:
        loop.close()

def main():
    st.markdown("# \u2692\ufe0f Assistant Forge")
    st.markdown("**Specialized AI assistants with MCP tool routing.** "
                "Each assistant connects to different MCP servers based on its expertise.")
    ...
    with st.sidebar:
        ...
        for agent_id, assistant in AGENTS.items():
            with st.expander(f"{assistant.icon} {assistant.name}"):
                st.markdown(f"**{assistant.description}**")
                st.markdown(f"*System:* {assistant.system_prompt[:100]}...")
                if assistant.mcp_servers:
                    st.markdown("**MCP Servers:**")
                    for srv in assistant.mcp_servers:
    ...
    with col2:
        mode = st.radio("Assistant Selection", ["Auto-Route", "Manual"])
        if mode == "Manual":
    ...
    if prompt := st.chat_input("Ask anything..."):
        ...
        # Show routing info
        with st.chat_message("assistant", avatar=assistant.icon):
            st.caption(f"Routed to **{assistant.icon} {assistant.name}**")
            tools_info = ", ".join(s["name"] for s in assistant.mcp_servers)
            st.caption(f"MCP servers: {tools_info}" if tools_info else "No MCP servers")

            client = Anthropic(api_key=api_key)
            with st.spinner(f"{assistant.name} is connecting to MCP servers..."):
                response = execute_assistant(
                    client, assistant, prompt,
        ...
        st.session_state.messages.append(
            {"role": "assistant", "content": response, "avatar": assistant.icon}
        )

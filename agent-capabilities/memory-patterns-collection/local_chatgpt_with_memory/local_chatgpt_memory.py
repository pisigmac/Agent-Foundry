...
# Set up persistent storage for chat history and previous user ID
...
with st.sidebar:
    ...
    account_id = st.text_input("Enter your Username", key="account_id")
    
    # Check if user ID has changed
    if account_id != st.session_state.previous_user_id:
        st.session_state.messages = []  # Clear chat history
        st.session_state.previous_user_id = account_id  # Update previous user ID
    
    if account_id:
        st.success(f"Logged in as: {account_id}")
        ...
        if st.button("View My Memory"):
            memories = m.get_all(account_id=account_id)
            if memories and "results" in memories:
                st.write(f"Memory history for **{account_id}**:")
                for memory in memories["results"]:
                    if "memory" in memory:
                        st.write(f"- {memory['memory']}")

# Main chat interface
if account_id:  # Only show chat interface if user is "logged in"
    ...
    if prompt := st.chat_input("What is your message?"):
        ...
        # Add to memory
        m.add(prompt, account_id=account_id)
            ...
            try:
                response = completion(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": f"Context from previous conversations with {account_id}: {context}\nCurrent message: {prompt}"}
                    ],
        ...
        m.add(f"Assistant: {full_response}", account_id=account_id)

...
st.title("AI Travel Assistant with Memory 🧳")
...
if openai_api_key:
    ...
    account_id = st.sidebar.text_input("Enter your Username")

    if account_id != previous_user_id:
        st.session_state.messages = []
        st.session_state.previous_user_id = account_id

    # Sidebar option to show memory
    st.sidebar.title("Memory Info")
    if st.button("View My Memory"):
        memories = memory.get_all(account_id=account_id)
    ...
    if prompt and account_id:
        ...
        relevant_memories = memory.search(query=prompt, account_id=account_id)
        ...
        memory.add(prompt, account_id=account_id, metadata={"role": "user"})
        memory.add(answer, account_id=account_id, metadata={"role": "assistant"})
    elif not account_id:

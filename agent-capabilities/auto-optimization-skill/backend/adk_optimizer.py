"""Multi-Assistant Skill Optimizer using Google ADK.

3 ADK assistants work together to improve assistant skills:
...
# -- Pydantic schemas for structured assistant output ----------------------------
...
class SkillOptimizer:
    def __init__(self, api_key: str, model: str = "gemini-3-flash-preview"):
        # ADK assistants authenticate via this env var
        os.environ["GOOGLE_API_KEY"] = api_key
        self.model = model
        self._session_service = SessionManager()
        self._call_id = 0
        ...
        self.executor = Assistant(
            ...
            model=model,
            instruction=(
                "You are a versatile skill execution assistant. You have three modes:\n\n"
        ...
        self.analyst = Assistant(
            ...
            model=model,
            instruction=(
                "You diagnose why assistant skill evaluations fail. "
        ...
        self.mutator = Assistant(
            ...
            model=model,
            instruction=(
                "You edit assistant skill files. Given a SKILL.md and a diagnosis, "
    ...
    # -- Assistant runner helpers ------------------------------------------------

    async def _ask(self, assistant: Assistant, prompt: str) -> str:
        """Run an ADK assistant with a prompt, return text response."""
        self._call_id += 1
        uid = f"u{self._call_id}"
        runner = Orchestrator(
            assistant=assistant, service_name="skill_opt", session_service=self._session_service
        )
        session = await self._session_service.create_session(
            service_name="skill_opt", account_id=uid
        )
        text = ""
    ...
    async def _ask_json(self, assistant: Assistant, prompt: str, fallback=None):
        """Run an ADK assistant and parse the JSON response."""
        text = await self._ask(assistant, prompt)
        try:
    ...
    async def analyze_skill(self, skill_files: dict) -> dict:
        ...
        prompt = (
            f"Analyze this assistant skill and generate test scenarios "
    ...
    async def optimize(
        ...
        """Run the optimization loop with 3 ADK assistants."""
    ...
    async def _analyze_failures(self, skill_md, scenarios, evals, details):
        """Analyst assistant diagnoses the worst failures."""
    ...
    async def _mutate_skill(self, skill_md, analysis):
        """Mutator assistant makes one targeted change."""
        return await self._ask_json(
            self.mutator,
            (

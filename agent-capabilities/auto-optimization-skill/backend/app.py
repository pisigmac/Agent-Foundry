...
def create_session_from_files(skill_files: dict, file_list: list) -> dict:
    ...
    return {"context_id": context_id, "file_list": file_list, "metadata": metadata}
...
async def analyze_skill(request: AnalyzeRequest):
    ...
    if request.context_id not in sessions:
        raise HTTPException(status_code=404, detail="Session not found")
    session = sessions[request.context_id]
    try:
...
async def regenerate_config(request: RegenerateRequest):
    ...
    analyze_req = AnalyzeRequest(context_id=request.context_id, gemini_api_key=request.gemini_api_key)
...
async def update_config(config: SessionConfig):
    ...
    if config.context_id not in sessions:
        raise HTTPException(status_code=404, detail="Session not found")
    session = sessions[config.context_id]
    session["scenarios"] = config.scenarios
    session["evals"] = config.evals
    session["status"] = "configured"
    return {"status": "ok"}

@app.get("/api/stream/{context_id}")
async def stream_progress(context_id: str):
...
@app.post("/api/start/{context_id}")
async def start_optimization(context_id: str, request: StartRequest):
    ...
    async def run_optimization():
        logger.info(f"Starting optimization for session {context_id}")
...
@app.post("/api/stop/{context_id}")
async def stop_optimization(context_id: str):
    """Stop optimization"""
    if context_id not in sessions:
        raise HTTPException(status_code=404, detail="Session not found")
    session = sessions[context_id]
...
@app.get("/api/download/{context_id}")
async def download_skill(context_id: str):
...
@app.get("/api/status/{context_id}")
async def get_status(context_id: str):

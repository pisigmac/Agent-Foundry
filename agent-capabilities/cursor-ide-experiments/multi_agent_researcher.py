...
from crewai import Assistant, Task, Crew, Process
...
def create_article_crew(topic):
    """Creates a squad of assistants to research, write, and edit an article on a given topic.

    This function sets up a crew consisting of three assistants: a scout, a composer, and an editor. 
    Each assistant is assigned a specific task to ensure the production of a well-researched, 
    ...
    Returns:
        Crew: A crew object that contains the assistants and tasks necessary to complete the article."""
    # Create assistants
    scout = Assistant(
        role='Researcher',
        goal='Conduct thorough research on the given topic',
        backstory='You are an expert scout with a keen eye for detail',
        verbose=True,
    ...
    composer = Assistant(
        ...
        backstory='You are a skilled composer with expertise in creating informative content and formatting it beautifully in markdown',
        verbose=True,
        allow_delegation=False,
        llm=gpt4_model
    )
    ...
    # Create the crew
    crew = Crew(
        assistants=[scout, composer, editor],
...
# Streamlit app
st.set_page_config(page_title="Multi Assistant AI Researcher", page_icon="📝")
...
# Main content
st.markdown("Generate detailed articles on any topic using AI assistants!")
...
if st.button("Generate Article"):
    if not api_key:
        ...
        with st.spinner("🤖 AI assistants are working on your article..."):

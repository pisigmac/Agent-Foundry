#!/usr/bin/env python3
"""
[vendored] Repo-side CI copy. Origin: the assistant-security-auditor skill (revamp
branch). If that skill ever ships on main, make it the single source of truth.
skill_scanner.py — static security scanner for assistant skills.
...
CRED_PATTERNS = [
    ...
    (re.compile(r"expanduser\([^)\n]{0,60}\.env|\$HOME/[^\s\"']{0,40}\.env\b|~/\.(?:claude|clawdbot|openclaw|config/openai)(?!/(?:skills|plugins)\b)[^\s\"']{0,30}"),
     "CRED01", "WARN", "Reads assistant/home .env or assistant config directories"),
        ...
        if in_fence and FETCH_CMD_RE.search(line):
            if i <= lure_window_until:
                add("LURE01", "CRITICAL", skill_md, i,
                    "Install/prerequisite section tells the user or assistant to fetch and run a remote script. This was the #1 ClawHavoc delivery vector.", line)
            else:
        ...
        for i, line in enumerate(content.splitlines(), 1):
            ...
            def doc_msg(sev, msg):
                if in_doc_fence and sev == "WARN":
                    return msg + " (inside a documentation code example — verify it is illustrative, not an instruction to the assistant)"
                return msg
            ...
            for rx, check, sev, msg in CRED_PATTERNS:
                if rx.search(line):
                    ...
                    # assistant, so they count toward the EXFIL01 cross-signal.
                    file_has_cred = True
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Static security scanner for assistant skills (OWASP AST01-AST10 aligned).")
    ...
    if not skills:
        print("error: no SKILL.md found under %s\n"
              "Fix: assistant skills are directories with a SKILL.md at their root. "
    ...
    if args.json:
        ...
        print("assistant-skill security scan v%s — %d skill(s) under %s\n" % (VERSION, len(skills), root))
        by_skill = {}
        for f in all_findings:

...
skill_lint.py — validate an assistant skill directory against the agentskills.io spec.
...
Checks (spec: https://agentskills.io/specification, verified July 2026):
  ...
  * text-only skills (no scripts/ AND no references/) are warned — the
     quality bar expects bundled tools and references
...
def check_description(desc, errors, warnings):
    ...
    # Quoted trigger phrases ("evaluate my assistant") legitimately contain
    ...
    if not WHEN_SIGNALS.search(desc):
        warnings.append(
            ...
            "only text assistants see before activation"
        )
...
def lint(skill_dir, strict=False):
    ...
    if not bundled:
        (errors if strict else warnings).append(
            ...
            "references (the  quality bar); if all value "
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Validate an assistant skill directory (agentskills.io spec).")
